...
class DeleteParam:
    ...
    def __init__(self, force=None, item_id=None, custom_tags=None, custom_num_tags=None):
        ...
        :type item_id: str
        :param custom_tags: 自定义字符标签，用于对服务实例中的数据进行custom_tags条件删除。格式为键值对{key:value}。 - key: 必须为服务实例custom_tags中已存在的key，可在创建服务实例时进行配置，或在更新服务实例时进行新增。 - value: 标签值列表，列表内多个标签值为“或”关系，即满足一个即可。列表长度范围为[1, 32]，标签值类型为字符串，字符长度范围为[1, 64]。
