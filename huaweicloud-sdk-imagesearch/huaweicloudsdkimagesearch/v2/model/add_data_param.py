...
class AddDataParam:
    ...
    def __init__(self, force=None, item_id=None, desc=None, custom_tags=None, custom_num_tags=None, image_base64=None, image_url=None, keywords=None, optional_params=None):
        ...
        :type desc: str
        :param custom_tags: 数据的自定义字符标签，可用于搜索时的过滤。格式为键值对{key:value}。 - key: 必须为服务实例custom_tags中已存在的key，可在创建服务实例时进行配置，或在更新服务实例时进行新增。 - value: 类型为字符串，字符长度范围为[1, 64]。
        :type custom_tags: dict(str, str)
        :param custom_num_tags: 数据的自定义数值标签，可用于搜索时的过滤。格式为键值对{key:value}。 - key: 必须为服务实例custom_num_tags中已存在的key，可在创建服务实例时进行配置，或在更新服务实例时进行新增。 - value: 类型为数值，格式为double。
        :type custom_num_tags: dict(str, float)
        :param image_base64: 图像文件的base64字符串，图像入库时，与image_url二选一。要求如下： - 格式：目前仅支持JPEG/JPG/PNG/BMP格式的图像。 - 尺寸：默认情况下，要求图像的最短边大于128px，最长边小于4096px。部分服务类型有特殊要求，可参见服务类型说明。 - 大小：图像文件大小要求不超过5M。 - 其他：图片中不能包含旋转信息。
