...
class MultichannelKvsClient(AbstractMultichannelKvsClient, KvsClientInterface):
    ...
    def create_table(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            ...
            try:
                return managed_client.client.create_table(request)
    ...
    def describe_table(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.describe_table(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def list_store(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.list_store(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def list_table(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.list_table(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def batch_get_kv(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.batch_get_kv(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def batch_write_kv(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.batch_write_kv(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def delete_kv(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.delete_kv(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def get_kv(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.get_kv(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def put_kv(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.put_kv(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def scan_kv(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.scan_kv(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def scan_skey_kv(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.scan_skey_kv(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def update_kv(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.update_kv(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
    ...
    def check_health(self, request):
        ...
        while retry_count < self._config.api_retry_count:
            managed_client = self.get_kvs_client_by_polling_with_retry(retry_count)
            try:
                return managed_client.client.check_health(request)
            except (ConnectionException, RequestTimeoutException) as e:
                managed_client.is_usable = False
                retry_count += 1
