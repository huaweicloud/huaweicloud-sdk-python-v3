# coding: utf-8

from huaweicloudsdkcore.utils.http_utils import sanitize_for_serialization


class LiveNotifyConfig:

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    sensitive_list = []

    openapi_types = {
        'notify_events': 'list[str]'
    }

    attribute_map = {
        'notify_events': 'notify_events'
    }

    def __init__(self, notify_events=None):
        r"""LiveNotifyConfig

        The model defined in huaweicloud sdk

        :param notify_events: **参数解释**： 启用通知事件列表。 **约束限制**： 不涉及 **默认取值**： 无。
        :type notify_events: list[str]
        """
        
        

        self._notify_events = None
        self.discriminator = None

        if notify_events is not None:
            self.notify_events = notify_events

    @property
    def notify_events(self):
        r"""Gets the notify_events of this LiveNotifyConfig.

        **参数解释**： 启用通知事件列表。 **约束限制**： 不涉及 **默认取值**： 无。

        :return: The notify_events of this LiveNotifyConfig.
        :rtype: list[str]
        """
        return self._notify_events

    @notify_events.setter
    def notify_events(self, notify_events):
        r"""Sets the notify_events of this LiveNotifyConfig.

        **参数解释**： 启用通知事件列表。 **约束限制**： 不涉及 **默认取值**： 无。

        :param notify_events: The notify_events of this LiveNotifyConfig.
        :type notify_events: list[str]
        """
        self._notify_events = notify_events

    def to_dict(self):
        result = {}

        for attr, _ in self.openapi_types.items():
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                if attr in self.sensitive_list:
                    result[attr] = "****"
                else:
                    result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        import simplejson as json
        return json.dumps(sanitize_for_serialization(self), ensure_ascii=False)

    def __repr__(self):
        """For `print`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, LiveNotifyConfig):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
