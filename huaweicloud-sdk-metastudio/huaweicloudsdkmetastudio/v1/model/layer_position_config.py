...
class LayerPositionConfig:
    ...
    def __init__(self, dx=None, dy=None, layer_index=None):
        ...
        :param dx: 图层图片左上角像素点的X轴位置值（相对背景图片）。 &gt; 横屏（16:9）背景图片像素为1920*1080；竖屏（9:16）背景图片像素为1080*1920。
        :type dx: int
        :param dy: 图层图片左上角像素点的Y轴位置值（相对背景图片）。 &gt; 横屏（16:9）背景图片像素为1920*1080；竖屏（9:16）背景图片像素为1080*1920。
        :type dy: int
