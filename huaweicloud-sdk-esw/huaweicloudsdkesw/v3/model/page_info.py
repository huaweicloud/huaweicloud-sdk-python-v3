...
class PageInfo:
    ...
    def __init__(self, next_marker=None, previous_marker=None, current_count=None):
        ...
        :param next_marker: - 参数解释：当前页的最后一条记录，最后一页时无next_marker字段。 - 约束限制：不涉及。 - 取值范围：不涉及。 - 默认取值：不涉及。
        :type next_marker: str
        :param previous_marker: - 参数解释：分页查询起始的资源ID，为空时查询第一页。 - 约束限制：不涉及。 - 取值范围：不涉及。 - 默认取值：不涉及。
        :type previous_marker: str
        :param current_count: - 参数解释：当前页记录总数。 - 约束限制：不涉及。 - 取值范围：不涉及。 - 默认取值：不涉及。
