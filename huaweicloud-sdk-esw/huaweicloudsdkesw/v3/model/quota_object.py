...
class QuotaObject:
    ...
    def __init__(self, type=None, quota=None, used=None, min=None, max=None):
        ...
        :param type: - 参数解释：配额类型。 - 约束限制：不涉及。 - 取值范围：instance。 - 默认取值：不涉及。
        :type type: str
        :param quota: - 参数解释：配额总量。 - 约束限制：不涉及。 - 取值范围：不涉及。 - 默认取值：不涉及。
        :type quota: int
        :param used: - 参数解释：已使用配额。 - 约束限制：不涉及。 - 取值范围：不涉及。 - 默认取值：不涉及。
        :type used: int
        :param min: - 参数解释：配额总量的最小值。 - 约束限制：不涉及。 - 取值范围：不涉及。 - 默认取值：不涉及。
        :type min: int
        :param max: - 参数解释：配额总量的最大值。 - 约束限制：不涉及。 - 取值范围：不涉及。 - 默认取值：不涉及。
        :type max: int
        """
