...
class Tag:
    ...
    def __init__(self, key=None, value=None):
        ...
        :param key: **参数说明**：标签键，标签的键可用字母(包含中文)、数字、空格和以下字符：下划线(_)点(.)冒号(:)等号(&#x3D;)加号(+)中划线(-)以及@，首尾不能有空格字符 
        :type key: str
        :param value: **参数说明**：标签值，可为空字符串和null，标签的值可用字母(包含中文)、数字、空格和以下字符：下划线(_)点(.)冒号(:)等号(&#x3D;)加号(+)中划线(-)以及@ 
        :type value: str
        """
