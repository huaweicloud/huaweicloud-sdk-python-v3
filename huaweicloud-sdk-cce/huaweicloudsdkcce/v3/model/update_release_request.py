# coding: utf-8

from huaweicloudsdkcore.utils.http_utils import sanitize_for_serialization


class UpdateReleaseRequest:

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    sensitive_list = []

    openapi_types = {
        'name': 'str',
        'namespace': 'str',
        'show_resources': 'str',
        'cluster_id': 'str',
        'body': 'UpdateReleaseReqBody'
    }

    attribute_map = {
        'name': 'name',
        'namespace': 'namespace',
        'show_resources': 'show_resources',
        'cluster_id': 'cluster_id',
        'body': 'body'
    }

    def __init__(self, name=None, namespace=None, show_resources=None, cluster_id=None, body=None):
        r"""UpdateReleaseRequest

        The model defined in huaweicloud sdk

        :param name: 模板实例名称
        :type name: str
        :param namespace: 模板实例所在的命名空间
        :type namespace: str
        :param show_resources: **参数解释：** 是否展示模板实例的资源信息。 **约束限制：** 不涉及 **取值范围：** 指定为“true”时展示模板实例的资源信息，不指定该参数时默认不展示。 **默认取值：** 无
        :type show_resources: str
        :param cluster_id: 集群ID，获取方式请参见[如何获取接口URI中参数](cce_02_0271.xml)。
        :type cluster_id: str
        :param body: Body of the UpdateReleaseRequest
        :type body: :class:`huaweicloudsdkcce.v3.UpdateReleaseReqBody`
        """
        
        

        self._name = None
        self._namespace = None
        self._show_resources = None
        self._cluster_id = None
        self._body = None
        self.discriminator = None

        self.name = name
        self.namespace = namespace
        if show_resources is not None:
            self.show_resources = show_resources
        self.cluster_id = cluster_id
        if body is not None:
            self.body = body

    @property
    def name(self):
        r"""Gets the name of this UpdateReleaseRequest.

        模板实例名称

        :return: The name of this UpdateReleaseRequest.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        r"""Sets the name of this UpdateReleaseRequest.

        模板实例名称

        :param name: The name of this UpdateReleaseRequest.
        :type name: str
        """
        self._name = name

    @property
    def namespace(self):
        r"""Gets the namespace of this UpdateReleaseRequest.

        模板实例所在的命名空间

        :return: The namespace of this UpdateReleaseRequest.
        :rtype: str
        """
        return self._namespace

    @namespace.setter
    def namespace(self, namespace):
        r"""Sets the namespace of this UpdateReleaseRequest.

        模板实例所在的命名空间

        :param namespace: The namespace of this UpdateReleaseRequest.
        :type namespace: str
        """
        self._namespace = namespace

    @property
    def show_resources(self):
        r"""Gets the show_resources of this UpdateReleaseRequest.

        **参数解释：** 是否展示模板实例的资源信息。 **约束限制：** 不涉及 **取值范围：** 指定为“true”时展示模板实例的资源信息，不指定该参数时默认不展示。 **默认取值：** 无

        :return: The show_resources of this UpdateReleaseRequest.
        :rtype: str
        """
        return self._show_resources

    @show_resources.setter
    def show_resources(self, show_resources):
        r"""Sets the show_resources of this UpdateReleaseRequest.

        **参数解释：** 是否展示模板实例的资源信息。 **约束限制：** 不涉及 **取值范围：** 指定为“true”时展示模板实例的资源信息，不指定该参数时默认不展示。 **默认取值：** 无

        :param show_resources: The show_resources of this UpdateReleaseRequest.
        :type show_resources: str
        """
        self._show_resources = show_resources

    @property
    def cluster_id(self):
        r"""Gets the cluster_id of this UpdateReleaseRequest.

        集群ID，获取方式请参见[如何获取接口URI中参数](cce_02_0271.xml)。

        :return: The cluster_id of this UpdateReleaseRequest.
        :rtype: str
        """
        return self._cluster_id

    @cluster_id.setter
    def cluster_id(self, cluster_id):
        r"""Sets the cluster_id of this UpdateReleaseRequest.

        集群ID，获取方式请参见[如何获取接口URI中参数](cce_02_0271.xml)。

        :param cluster_id: The cluster_id of this UpdateReleaseRequest.
        :type cluster_id: str
        """
        self._cluster_id = cluster_id

    @property
    def body(self):
        r"""Gets the body of this UpdateReleaseRequest.

        :return: The body of this UpdateReleaseRequest.
        :rtype: :class:`huaweicloudsdkcce.v3.UpdateReleaseReqBody`
        """
        return self._body

    @body.setter
    def body(self, body):
        r"""Sets the body of this UpdateReleaseRequest.

        :param body: The body of this UpdateReleaseRequest.
        :type body: :class:`huaweicloudsdkcce.v3.UpdateReleaseReqBody`
        """
        self._body = body

    def to_dict(self):
        result = {}

        for attr, _ in self.openapi_types.items():
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                if attr in self.sensitive_list:
                    result[attr] = "****"
                else:
                    result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        import simplejson as json
        return json.dumps(sanitize_for_serialization(self), ensure_ascii=False)

    def __repr__(self):
        """For `print`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, UpdateReleaseRequest):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
