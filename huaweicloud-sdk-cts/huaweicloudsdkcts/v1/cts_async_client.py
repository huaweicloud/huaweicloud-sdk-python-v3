...
class CtsAsyncClient(Client):
    ...
    def create_tracker_async(self, request):
        ...
        云审计服务开通后系统会自动创建一个追踪器，用来关联系统记录的所有操作。目前，一个云账户在一个Region下仅支持创建一个追踪器。 云审计服务支持在管理控制台查询近7天内的操作记录。如需保存更长时间的操作记录，您可以在创建追踪器之后通过对象存储服务（Object Storage Service，以下简称OBS）将操作记录实时保存至OBS桶中。
    ...
    def list_traces_v2_async(self, request):
        """查询事件列表(v2.0)
    ...
    def list_tracker_async(self, request):
        """查询追踪器
