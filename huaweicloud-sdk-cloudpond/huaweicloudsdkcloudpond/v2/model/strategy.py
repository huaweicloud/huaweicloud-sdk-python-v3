# coding: utf-8

import six

from huaweicloudsdkcore.utils.http_utils import sanitize_for_serialization


class Strategy:

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    sensitive_list = []

    openapi_types = {
        'status': 'OperationStatus',
        'scene': 'Scene',
        'effect': 'Effect'
    }

    attribute_map = {
        'status': 'status',
        'scene': 'scene',
        'effect': 'effect'
    }

    def __init__(self, status=None, scene=None, effect=None):
        r"""Strategy

        The model defined in huaweicloud sdk

        :param status: 
        :type status: :class:`huaweicloudsdkcloudpond.v2.OperationStatus`
        :param scene: 
        :type scene: :class:`huaweicloudsdkcloudpond.v2.Scene`
        :param effect: 
        :type effect: :class:`huaweicloudsdkcloudpond.v2.Effect`
        """
        
        

        self._status = None
        self._scene = None
        self._effect = None
        self.discriminator = None

        if status is not None:
            self.status = status
        if scene is not None:
            self.scene = scene
        if effect is not None:
            self.effect = effect

    @property
    def status(self):
        r"""Gets the status of this Strategy.

        :return: The status of this Strategy.
        :rtype: :class:`huaweicloudsdkcloudpond.v2.OperationStatus`
        """
        return self._status

    @status.setter
    def status(self, status):
        r"""Sets the status of this Strategy.

        :param status: The status of this Strategy.
        :type status: :class:`huaweicloudsdkcloudpond.v2.OperationStatus`
        """
        self._status = status

    @property
    def scene(self):
        r"""Gets the scene of this Strategy.

        :return: The scene of this Strategy.
        :rtype: :class:`huaweicloudsdkcloudpond.v2.Scene`
        """
        return self._scene

    @scene.setter
    def scene(self, scene):
        r"""Sets the scene of this Strategy.

        :param scene: The scene of this Strategy.
        :type scene: :class:`huaweicloudsdkcloudpond.v2.Scene`
        """
        self._scene = scene

    @property
    def effect(self):
        r"""Gets the effect of this Strategy.

        :return: The effect of this Strategy.
        :rtype: :class:`huaweicloudsdkcloudpond.v2.Effect`
        """
        return self._effect

    @effect.setter
    def effect(self, effect):
        r"""Sets the effect of this Strategy.

        :param effect: The effect of this Strategy.
        :type effect: :class:`huaweicloudsdkcloudpond.v2.Effect`
        """
        self._effect = effect

    def to_dict(self):
        """Returns the model properties as a dict"""
        result = {}

        for attr, _ in six.iteritems(self.openapi_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                if attr in self.sensitive_list:
                    result[attr] = "****"
                else:
                    result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        import simplejson as json
        if six.PY2:
            import sys
            reload(sys)
            sys.setdefaultencoding("utf-8")
        return json.dumps(sanitize_for_serialization(self), ensure_ascii=False)

    def __repr__(self):
        """For `print`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, Strategy):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
