# coding: utf-8

import six

from huaweicloudsdkcore.utils.http_utils import sanitize_for_serialization


class SmartConnectValidateEntity:

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    sensitive_list = []

    openapi_types = {
        'task': 'SmartConnectTaskRespSourceConfig',
        'type': 'str'
    }

    attribute_map = {
        'task': 'task',
        'type': 'type'
    }

    def __init__(self, task=None, type=None):
        r"""SmartConnectValidateEntity

        The model defined in huaweicloud sdk

        :param task: 
        :type task: :class:`huaweicloudsdkkafka.v2.SmartConnectTaskRespSourceConfig`
        :param type: **参数解释**： Smart Connect任务类型。 **取值范围**： - OBS_SINK：转储。 - KAFKA_REPLICATOR_SOURCE：Kafka数据复制。
        :type type: str
        """
        
        

        self._task = None
        self._type = None
        self.discriminator = None

        if task is not None:
            self.task = task
        if type is not None:
            self.type = type

    @property
    def task(self):
        r"""Gets the task of this SmartConnectValidateEntity.

        :return: The task of this SmartConnectValidateEntity.
        :rtype: :class:`huaweicloudsdkkafka.v2.SmartConnectTaskRespSourceConfig`
        """
        return self._task

    @task.setter
    def task(self, task):
        r"""Sets the task of this SmartConnectValidateEntity.

        :param task: The task of this SmartConnectValidateEntity.
        :type task: :class:`huaweicloudsdkkafka.v2.SmartConnectTaskRespSourceConfig`
        """
        self._task = task

    @property
    def type(self):
        r"""Gets the type of this SmartConnectValidateEntity.

        **参数解释**： Smart Connect任务类型。 **取值范围**： - OBS_SINK：转储。 - KAFKA_REPLICATOR_SOURCE：Kafka数据复制。

        :return: The type of this SmartConnectValidateEntity.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        r"""Sets the type of this SmartConnectValidateEntity.

        **参数解释**： Smart Connect任务类型。 **取值范围**： - OBS_SINK：转储。 - KAFKA_REPLICATOR_SOURCE：Kafka数据复制。

        :param type: The type of this SmartConnectValidateEntity.
        :type type: str
        """
        self._type = type

    def to_dict(self):
        """Returns the model properties as a dict"""
        result = {}

        for attr, _ in six.iteritems(self.openapi_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                if attr in self.sensitive_list:
                    result[attr] = "****"
                else:
                    result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        import simplejson as json
        if six.PY2:
            import sys
            reload(sys)
            sys.setdefaultencoding("utf-8")
        return json.dumps(sanitize_for_serialization(self), ensure_ascii=False)

    def __repr__(self):
        """For `print`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, SmartConnectValidateEntity):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
