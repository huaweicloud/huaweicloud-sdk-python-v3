...
class ServiceUpdateRequest:
    ...
    openapi_types = {
        ...
        'runtime_config': 'RuntimeConfigUpdateRequest',
    ...
    def __init__(self, id=None, name=None, deploy_timeout_minutes=None, version=None, description=None, group_configs=None, runtime_config=None, upgrade_config=None, lts_strategy=None, log_configs=None, tags=None, workspace_id=None, schedule=None, custom_metrics_path=None, task_type=None):
        ...
        :param id: **参数解释：** 服务ID **约束限制：** 不填保留原有值。 **取值范围：** 不涉及
        :type id: str
        :param name: **参数解释：** 服务名称。 **约束限制：** 不填保留原有值。 **取值范围：** 支持1-128个字符，可以包含字母、汉字、数字、连字符和下划线。 **默认取值：** 不涉及。
        :type name: str
        :param deploy_timeout_minutes: **参数解释：** 服务部署超时时间，integer类型，取值在1~300。 **约束限制：** 不填保留原有值。 **取值范围：** [0, 300]。 **默认取值：** 不涉及。
        :type deploy_timeout_minutes: int
        :param version: **参数解释：** 服务版本，数据库中如果存在相同版本号，将会报错（仅修改描述的场景除外）。 **约束限制：** 不填保留原有值。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
        :type version: str
        :param description: **参数解释：** 非必填，仅更新描述的场景直接修改对应version的数据库字段，不新增版本号。 **约束限制：** 不填保留原有值。 **取值范围：** 不涉及 **默认取值：** 不涉及
        :type description: str
        :param group_configs: **参数解释：** 仅修改服务时不需传，兼容部署分离之前版本。 **约束限制：** 不填保留原有值，group_configs的最大元素数量为1。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
        :type group_configs: list[:class:`huaweicloudsdkmodelarts.v1.GroupConfig`]
        :param runtime_config: 
        :type runtime_config: :class:`huaweicloudsdkmodelarts.v1.RuntimeConfigUpdateRequest`
        :param upgrade_config: 
        :type upgrade_config: :class:`huaweicloudsdkmodelarts.v1.UpgradeConfig`
        :param lts_strategy: **参数解释：** 日志策略。 **约束限制：** 不填保留原有值。 **取值范围：** - POOL：使用资源池日志插件配置的日志流。 - AUTO_CREATE：自动创建日志流。 - DEFAULT: 由系统决定日志策略 **默认取值：** 不涉及。
        :type lts_strategy: str
        :param log_configs: **参数解释：** 服务日志配置。 **约束限制：** 1.不填保留原有值 2.数量上限为[3](tag:hws,hws_hk,fcs,fcs_super)[2](tag:hcs,hcs_sm)个，且每种类型只可配置一个。
        :type log_configs: list[:class:`huaweicloudsdkmodelarts.v1.LtsConfiguration`]
        :param tags: **参数解释：** 服务标签,上限20个 **约束限制：** 不填保留原有值。 **取值范围：** 不涉及 **默认取值：** 不涉及
        :type tags: str
        ...
        :param task_type: **参数解释：** 模型类型。 **约束限制：** 不填保留原有值。 **取值范围：** - TEXT_GENERATION：文本生成 - IMAGE_UNDERSTANDING：图像理解 - VIDEO_GENERATION：视频生成 - IMAGE_GENERATION：图像生成 - RERANK：重排序 - VECTOR_MODEL：向量模型 - EMBEDDING：Embedding嵌入
        :type task_type: str
        """
    ...
    def runtime_config(self):
        ...
        :rtype: :class:`huaweicloudsdkmodelarts.v1.RuntimeConfigUpdateRequest`
        """
