...
class DeleteInferApiKeyRequest:
    ...
    @property
    def key_id(self):
        r"""Gets the key_id of this DeleteInferApiKeyRequest.
        ...
        **参数解释：** apikey_id，在[创建API_KEY](CreateInferApiKey.xml)时即可在返回体中获取，也可通过[查询api-keys列表](ListInferApiKeys.xml)获取当前用户拥有的apikey，其中key_id字段即为apikey_id。 **约束限制：** 不涉及。 **取值范围：** apikey_id只能由英文小写字母、数字组成，且长度为32个字符。 **默认取值：** 不涉及。
        ...
        :return: The key_id of this DeleteInferApiKeyRequest.
        :rtype: str
        """
        return self._key_id
    ...
    def key_id(self, key_id):
        r"""Sets the key_id of this DeleteInferApiKeyRequest.

        **参数解释：** apikey_id，在[创建API_KEY](CreateInferApiKey.xml)时即可在返回体中获取，也可通过[查询api-keys列表](ListInferApiKeys.xml)获取当前用户拥有的apikey，其中key_id字段即为apikey_id。 **约束限制：** 不涉及。 **取值范围：** apikey_id只能由英文小写字母、数字组成，且长度为32个字符。 **默认取值：** 不涉及。

        :param key_id: The key_id of this DeleteInferApiKeyRequest.
        :type key_id: str
        """
        self._key_id = key_id
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, DeleteInferApiKeyRequest):
            return False
