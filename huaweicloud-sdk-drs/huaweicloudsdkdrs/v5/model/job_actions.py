...
class JobActions:
    ...
    def __init__(self, available_actions=None, unavailable_actions=None, current_action=None):
        ...
        :param unavailable_actions: 任务可操作命令集合。 - CREATE：创建任务 - CHOOSE_OBJECT：选择对象，任务增量中再编辑 - PRE_CHECK：预检查 - CHANGE_MODE：修改任务模式 - FREE_RESOURCE：释放资源 - MODIFY_DB_CONFIG：修改数据库配置 - RESET_DB_PWD：重置数据库密码（源库、目标库） - MODIFY_CONFIGURATION：修改任务配置 - PAUSE：暂停任务 - START：启动任务 - CHANGE：修改任务 - RETRY：重试任务 - RESET：重置任务 - DELETE：删除任务 - QUERY_PRE_CHECK：预检查 - SWITCH_OVER：容灾倒换 - START_INCR：CASSANDRA启动增量任务 - MODIFY_TASK_NUMBER：CASSANDRA修改线程数配置 - CONTINUE_JOB：oracle-GaussDB分布式:启动失败或者停止的任务 - STOP_JOB：oracle-GaussDB分布式:停止任务 - CONTINUE_CAPTURE：启动抓取 - STOP_CAPTURE：停止抓取 - CONTINUE_APPLY：启动回放 - STOP_APPLY：停止回放 - PAY_ORDER：包年包月支付订单 - UNSUBSCRIBE：包年包月退订 - TO_PERIOD：转包周期 - TO_RENEW：包周期续费 - ORDER_INFO：订单详情 - CHANGE_FLAVOR：规格变更 - CLONE：克隆任务
    ...
    def available_actions(self):
        r"""Gets the available_actions of this JobActions.
    ...
    def available_actions(self, available_actions):
        r"""Sets the available_actions of this JobActions.
    ...
    def unavailable_actions(self):
        r"""Gets the unavailable_actions of this JobActions.
    ...
    def unavailable_actions(self, unavailable_actions):
        r"""Sets the unavailable_actions of this JobActions.
    ...
    def current_action(self):
        r"""Gets the current_action of this JobActions.
    ...
    def current_action(self, current_action):
        r"""Sets the current_action of this JobActions.
