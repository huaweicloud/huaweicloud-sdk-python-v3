...
class ContinueRollbackStackResponse(SdkResponse):
    ...
    def __init__(self, deployment_id=None):
        ...
        :param deployment_id: 继续回滚触发部署生成的唯一的deployment_id，由资源编排服务生成，通常为UUID  接受请求，进行异步处理。可以调用GetStackMetadata来获取异步请求的部署状态  **注意：** * 资源编排服务最终会将异步部署请求排队，在服务端空闲的情况下逐个处理。用户最大等待时长为6小时
    ...
    def to_dict(self):
        import warnings
        warnings.warn("ContinueRollbackStackResponse.to_dict() is deprecated and no longer maintained, "
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ContinueRollbackStackResponse):
            return False
