...
class VpcMemberModify:
    ...
    def __init__(self, members=None, member_group_name=None):
        ...
        :param member_group_name: 需要修改的后端服务器组  不传时使用members中的定义对VPC通道后端进行全量覆盖修改。  传入时，只对members中对应后端服务器组的后端实例进行处理，其他后端服务器组的入参会被忽略。例如：member_group_name&#x3D;primary时，只处理members中后端服务器组为primary的后端实例。
