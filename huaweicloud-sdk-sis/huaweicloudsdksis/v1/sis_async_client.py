...
class SisAsyncClient(Client):
    ...
    def collect_transcriber_job_async(self, request):
        ...
        :param CollectTranscriberJobRequest request
    ...
    def push_transcriber_jobs_async(self, request):
        ...
        **录音文件识别** 录音文件识别接口，用于识别长录音文件，录音文件放在华为云OBS（对象存储服务）上。  由于录音文件识别通常会需要较长的时间，因此识别是异步的，也即接口分为创建识别任务和查询任务状态两个接口。创建识别任务接口创建任务完成后返回，然后用户通过调用查询任务状态接口来获得转写状态和结果。  **功能介绍** 该接口用于提交录音文件识别任务，其中录音文件保存在用户的OBS桶中。用户开通录音识别服务时，需授权录音文件引擎读取用户OBS桶权限。  接口约束 录音时长不超过5小时，文件大小不超过300M，识别结果保存72小时（从识别完成的时间算起）。72小时后如果再访问，将会返回 \"task id is not found\"错误。
        ...
        :param PushTranscriberJobsRequest request
    ...
    def recognize_flash_asr_async(self, request):
        ...
        :param RecognizeFlashAsrRequest request
    ...
    def recognize_short_audio_async(self, request):
        ...
        :param RecognizeShortAudioRequest request
    ...
    def run_audio_assessment_async(self, request):
        """语音评测

        口语评测接口，基于一小段朗读语音和预期文本，评价朗读者发音质量。当前仅支持华北-北京四。

        :param RunAudioAssessmentRequest request
    ...
    def run_multi_modal_assessment_async(self, request):
        """多模态评测

        多模态评测接口，根据朗读视频数据、视频对应的音频数据和试题文本，综合给出朗读者口语的评测分数。当前仅支持华北-北京四。

        :param RunMultiModalAssessmentRequest request
    ...
    def show_vocabularies_async(self, request):
        ...
        :param ShowVocabulariesRequest request
    ...
    def show_vocabulary_async(self, request):
        ...
        :param ShowVocabularyRequest request
