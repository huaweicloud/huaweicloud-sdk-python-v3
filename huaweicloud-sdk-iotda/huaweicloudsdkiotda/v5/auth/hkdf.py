...
def get_der_key_sha256(access_key: str, secret_key: str, info: str):
    if not access_key or not secret_key:
        return None
    try:
        tmp_key: bytes = extract(secret_key, access_key, HMAC_ALGORITHM)
        der_secret_key: bytes = expand(tmp_key, info.encode(UTF_8), HMAC_ALGORITHM, DERIVATION_KEY_LENGTH, EXPAND_CEIL)
...
def extract(ikm: str, salt: str, hmac_algorithm) -> bytes:
    if not salt:
...
def expand(prk: bytes, info: bytes, hmac_algorithm, okm_len: int, ceil: int):
    if ceil == 1:
...
def expand_first(prk: bytes, info: bytes, hmac_algorithm):
    result = copy.deepcopy(info)
    result = result + bytearray((1,))
    return hmac_sha256(prk, result, hmac_algorithm)


def expand_once(prk: bytes, info: bytes, pre_tmp: bytes, i: int, hmac_algorithm):
