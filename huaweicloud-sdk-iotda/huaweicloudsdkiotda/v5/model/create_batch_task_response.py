...
class CreateBatchTaskResponse(SdkResponse):
    ...
    def __init__(self, task_id=None, task_name=None, task_type=None, task_mode=None, need_confirm=None, task_ext_info=None, targets=None, targets_filter=None, document=None, task_policy=None, status=None, status_desc=None, task_progress=None, create_time=None):
        ...
        :param status: 批量任务的状态，可选参数，取值范围：Success|Fail|Processing|PartialSuccess|Stopped|Waitting|Initializing|Stopping。 - Initializing: 初始化中。 - Waitting: 等待中。 - Processing: 执行中。 - Success: 成功。 - Fail: 失败。 - PartialSuccess: 部分成功。 - Stopped: 停止。 - Stopping 停止中。
        :type status: str
    ...
    def task_mode(self):
        ...
        **参数说明**：批量任务的模式，当前只支持网关模式，当task_type为firmwareUpgrade，softwareUpgrade支持该参数。软固件升级的场景下，若升级的设备为某个网关的子设备，则平台下发获取版本信息通知和平台下发升级通知将携带task_id（软固件升级批量任务的任务ID）和sub_device_count（批量任务中网关设备包含的升级子设备数量）字段。 **取值范围**：GATEWAY: 网关模式。
        ...
        :return: The task_mode of this CreateBatchTaskResponse.
        :rtype: str
        """
    ...
    def task_mode(self, task_mode):
        ...
        **参数说明**：批量任务的模式，当前只支持网关模式，当task_type为firmwareUpgrade，softwareUpgrade支持该参数。软固件升级的场景下，若升级的设备为某个网关的子设备，则平台下发获取版本信息通知和平台下发升级通知将携带task_id（软固件升级批量任务的任务ID）和sub_device_count（批量任务中网关设备包含的升级子设备数量）字段。 **取值范围**：GATEWAY: 网关模式。
        ...
        :param task_mode: The task_mode of this CreateBatchTaskResponse.
        :type task_mode: str
        """
    ...
    def need_confirm(self):
        r"""Gets the need_confirm of this CreateBatchTaskResponse.
        ...
        **参数说明**：当需要自主控制该任务时设置此参数，当task_type为firmwareUpgrade，softwareUpgrade,moduleUpgrade支持该参数。软固件升级的场景下，当need_confirm为true时，任务分发后所有子任务一直为waitting状态，需要调确认接口后才变为processing状态，任务开始分发，不设置或者设置为false时子任务自动分发。 **取值范围**：true: 需要确认才分发子任务，false: 不需要确认自动分发子任务。
        ...
        :return: The need_confirm of this CreateBatchTaskResponse.
        :rtype: bool
        """
    ...
    def need_confirm(self, need_confirm):
        r"""Sets the need_confirm of this CreateBatchTaskResponse.

        **参数说明**：当需要自主控制该任务时设置此参数，当task_type为firmwareUpgrade，softwareUpgrade,moduleUpgrade支持该参数。软固件升级的场景下，当need_confirm为true时，任务分发后所有子任务一直为waitting状态，需要调确认接口后才变为processing状态，任务开始分发，不设置或者设置为false时子任务自动分发。 **取值范围**：true: 需要确认才分发子任务，false: 不需要确认自动分发子任务。

        :param need_confirm: The need_confirm of this CreateBatchTaskResponse.
        :type need_confirm: bool
        """
    ...
    def task_ext_info(self):
        ...
        **参数说明**：批量任务额外扩展信息，当task_type为firmwareUpgrade，softwareUpgrade支持该参数。软固件升级的场景下，平台下发获取版本信息通知和平台下发升级通知将携带该字段。 **取值范围**：最长不超过512个字符。
        ...
        :return: The task_ext_info of this CreateBatchTaskResponse.
        :rtype: object
        """
    ...
    def task_ext_info(self, task_ext_info):
        ...
        **参数说明**：批量任务额外扩展信息，当task_type为firmwareUpgrade，softwareUpgrade支持该参数。软固件升级的场景下，平台下发获取版本信息通知和平台下发升级通知将携带该字段。 **取值范围**：最长不超过512个字符。
        ...
        :param task_ext_info: The task_ext_info of this CreateBatchTaskResponse.
        :type task_ext_info: object
        """
    ...
    def to_dict(self):
        import warnings
        warnings.warn("CreateBatchTaskResponse.to_dict() is deprecated and no longer maintained, "
