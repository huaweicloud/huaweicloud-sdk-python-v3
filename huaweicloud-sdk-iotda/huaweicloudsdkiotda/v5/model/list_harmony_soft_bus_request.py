...
class ListHarmonySoftBusRequest:
    ...
    def __init__(self, instance_id=None, limit=None, marker=None, offset=None, group_id=None, app_id=None):
        r"""ListHarmonySoftBusRequest
    ...
    def instance_id(self):
        r"""Gets the instance_id of this ListHarmonySoftBusRequest.

        **参数说明**：实例ID。物理多租下各实例的唯一标识，一般华为云租户无需携带该参数，仅在物理多租场景下从管理面访问API时需要携带该参数。您可以在IoTDA管理控制台界面，选择左侧导航栏“总览”页签查看当前实例的ID

        :return: The instance_id of this ListHarmonySoftBusRequest.
        :rtype: str
        """
    ...
    def instance_id(self, instance_id):
        r"""Sets the instance_id of this ListHarmonySoftBusRequest.

        **参数说明**：实例ID。物理多租下各实例的唯一标识，一般华为云租户无需携带该参数，仅在物理多租场景下从管理面访问API时需要携带该参数。您可以在IoTDA管理控制台界面，选择左侧导航栏“总览”页签查看当前实例的ID

        :param instance_id: The instance_id of this ListHarmonySoftBusRequest.
        :type instance_id: str
        """
    ...
    @property
    def limit(self):
        r"""Gets the limit of this ListHarmonySoftBusRequest.

        **参数说明**：分页查询时每页显示的记录数。 **取值范围**：1-50的整数，默认值为10。

        :return: The limit of this ListHarmonySoftBusRequest.
        :rtype: int
        """
        return self._limit
    ...
    def limit(self, limit):
        r"""Sets the limit of this ListHarmonySoftBusRequest.

        **参数说明**：分页查询时每页显示的记录数。 **取值范围**：1-50的整数，默认值为10。

        :param limit: The limit of this ListHarmonySoftBusRequest.
        :type limit: int
        """
        self._limit = limit
    ...
    @property
    def marker(self):
        r"""Gets the marker of this ListHarmonySoftBusRequest.
        ...
        **参数说明**：上一次分页查询结果中最后一条记录的ID，在上一次分页查询时由物联网平台返回获得。分页查询时物联网平台是按marker也就是记录ID降序查询的，越新的数据记录ID也会越大。若填写marker，则本次只查询记录ID小于marker的数据记录。若不填写，则从记录ID最大也就是最新的一条数据开始查询。如果需要依次查询所有数据，则每次查询时必须填写上一次查询响应中的marker值。 **取值范围**：长度为24的十六进制字符串，默认值为ffffffffffffffffffffffff。
        ...
        :return: The marker of this ListHarmonySoftBusRequest.
        :rtype: str
        """
        return self._marker
    ...
    def marker(self, marker):
        r"""Sets the marker of this ListHarmonySoftBusRequest.

        **参数说明**：上一次分页查询结果中最后一条记录的ID，在上一次分页查询时由物联网平台返回获得。分页查询时物联网平台是按marker也就是记录ID降序查询的，越新的数据记录ID也会越大。若填写marker，则本次只查询记录ID小于marker的数据记录。若不填写，则从记录ID最大也就是最新的一条数据开始查询。如果需要依次查询所有数据，则每次查询时必须填写上一次查询响应中的marker值。 **取值范围**：长度为24的十六进制字符串，默认值为ffffffffffffffffffffffff。

        :param marker: The marker of this ListHarmonySoftBusRequest.
        :type marker: str
        """
        self._marker = marker
    ...
    @property
    def offset(self):
        r"""Gets the offset of this ListHarmonySoftBusRequest.
        ...
        **参数说明**：表示从marker后偏移offset条记录开始查询。当offset为0时，表示从marker后第一条记录开始输出。限制offset最大值是出于API性能考虑，您可以搭配marker使用该参数实现翻页，例如每页50条记录，1-11页内都可以直接使用offset跳转到指定页，但到11页后，由于offset限制为500，您需要使用第11页返回的marker作为下次查询的marker，以实现翻页到12-22页。 **取值范围**：0-500的整数，默认为0。
        ...
        :return: The offset of this ListHarmonySoftBusRequest.
        :rtype: int
        """
        return self._offset
    ...
    def offset(self, offset):
        r"""Sets the offset of this ListHarmonySoftBusRequest.

        **参数说明**：表示从marker后偏移offset条记录开始查询。当offset为0时，表示从marker后第一条记录开始输出。限制offset最大值是出于API性能考虑，您可以搭配marker使用该参数实现翻页，例如每页50条记录，1-11页内都可以直接使用offset跳转到指定页，但到11页后，由于offset限制为500，您需要使用第11页返回的marker作为下次查询的marker，以实现翻页到12-22页。 **取值范围**：0-500的整数，默认为0。

        :param offset: The offset of this ListHarmonySoftBusRequest.
        :type offset: int
        """
        self._offset = offset
    ...
    @property
    def group_id(self):
        r"""Gets the group_id of this ListHarmonySoftBusRequest.

        **参数说明**：设备组ID，用于唯一标识一个设备组，在创建设备组时由物联网平台分配。 **取值范围**：长度不超过36，十六进制字符串和连接符（-）的组合。

        :return: The group_id of this ListHarmonySoftBusRequest.
        :rtype: str
        """
    ...
    def group_id(self, group_id):
        r"""Sets the group_id of this ListHarmonySoftBusRequest.

        **参数说明**：设备组ID，用于唯一标识一个设备组，在创建设备组时由物联网平台分配。 **取值范围**：长度不超过36，十六进制字符串和连接符（-）的组合。

        :param group_id: The group_id of this ListHarmonySoftBusRequest.
        :type group_id: str
        """
    ...
    def app_id(self, app_id):
        r"""Sets the app_id of this ListHarmonySoftBusRequest.

        **参数说明**：资源空间ID。此参数为非必选参数，存在多资源空间的用户需要使用该接口时，可以携带该参数查询指定资源空间下的产品列表，不携带该参数则会查询该用户下所有产品列表。 **取值范围**：长度不超过36，只允许字母、数字、下划线（_）、连接符（-）的组合。

        :param app_id: The app_id of this ListHarmonySoftBusRequest.
        :type app_id: str
        """
        self._app_id = app_id
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ListHarmonySoftBusRequest):
            return False
