# coding: utf-8

from huaweicloudsdkcore.utils.http_utils import sanitize_for_serialization


class ProtectedBranchProtectedActionBodyDto:

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    sensitive_list = []

    openapi_types = {
        'enable': 'bool',
        'user_ids': 'list[int]',
        'user_team_ids': 'list[int]',
        'related_role_ids': 'list[str]',
        'action': 'str',
        'addition_switchers': 'list[ProtectedActionSwitcher]'
    }

    attribute_map = {
        'enable': 'enable',
        'user_ids': 'user_ids',
        'user_team_ids': 'user_team_ids',
        'related_role_ids': 'related_role_ids',
        'action': 'action',
        'addition_switchers': 'addition_switchers'
    }

    def __init__(self, enable=None, user_ids=None, user_team_ids=None, related_role_ids=None, action=None, addition_switchers=None):
        r"""ProtectedBranchProtectedActionBodyDto

        The model defined in huaweicloud sdk

        :param enable: **参数解释：** 是否启用。 **约束限制：** 不涉及。 **取值范围：** - true，开启规则限制 - false，关闭规则限制 **默认取值：** 不涉及。
        :type enable: bool
        :param user_ids: **参数解释：** 用户ID列表。 **约束限制：** 不涉及。 **取值范围：** Integer **默认取值：** 不涉及。
        :type user_ids: list[int]
        :param user_team_ids: **参数解释：** 成员组ID列表。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
        :type user_team_ids: list[int]
        :param related_role_ids: **参数解释：** 关联角色ID列表。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
        :type related_role_ids: list[str]
        :param action: **参数解释：** 事件名称。 **约束限制：** action中必传此字段。 **取值范围：** - push，推送 。 - merge，合并 。 **默认取值：** 不涉及。
        :type action: str
        :param addition_switchers: **参数解释：** 附加开关。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
        :type addition_switchers: list[:class:`huaweicloudsdkcodeartsrepo.v4.ProtectedActionSwitcher`]
        """
        
        

        self._enable = None
        self._user_ids = None
        self._user_team_ids = None
        self._related_role_ids = None
        self._action = None
        self._addition_switchers = None
        self.discriminator = None

        if enable is not None:
            self.enable = enable
        if user_ids is not None:
            self.user_ids = user_ids
        if user_team_ids is not None:
            self.user_team_ids = user_team_ids
        if related_role_ids is not None:
            self.related_role_ids = related_role_ids
        if action is not None:
            self.action = action
        if addition_switchers is not None:
            self.addition_switchers = addition_switchers

    @property
    def enable(self):
        r"""Gets the enable of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 是否启用。 **约束限制：** 不涉及。 **取值范围：** - true，开启规则限制 - false，关闭规则限制 **默认取值：** 不涉及。

        :return: The enable of this ProtectedBranchProtectedActionBodyDto.
        :rtype: bool
        """
        return self._enable

    @enable.setter
    def enable(self, enable):
        r"""Sets the enable of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 是否启用。 **约束限制：** 不涉及。 **取值范围：** - true，开启规则限制 - false，关闭规则限制 **默认取值：** 不涉及。

        :param enable: The enable of this ProtectedBranchProtectedActionBodyDto.
        :type enable: bool
        """
        self._enable = enable

    @property
    def user_ids(self):
        r"""Gets the user_ids of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 用户ID列表。 **约束限制：** 不涉及。 **取值范围：** Integer **默认取值：** 不涉及。

        :return: The user_ids of this ProtectedBranchProtectedActionBodyDto.
        :rtype: list[int]
        """
        return self._user_ids

    @user_ids.setter
    def user_ids(self, user_ids):
        r"""Sets the user_ids of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 用户ID列表。 **约束限制：** 不涉及。 **取值范围：** Integer **默认取值：** 不涉及。

        :param user_ids: The user_ids of this ProtectedBranchProtectedActionBodyDto.
        :type user_ids: list[int]
        """
        self._user_ids = user_ids

    @property
    def user_team_ids(self):
        r"""Gets the user_team_ids of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 成员组ID列表。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。

        :return: The user_team_ids of this ProtectedBranchProtectedActionBodyDto.
        :rtype: list[int]
        """
        return self._user_team_ids

    @user_team_ids.setter
    def user_team_ids(self, user_team_ids):
        r"""Sets the user_team_ids of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 成员组ID列表。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。

        :param user_team_ids: The user_team_ids of this ProtectedBranchProtectedActionBodyDto.
        :type user_team_ids: list[int]
        """
        self._user_team_ids = user_team_ids

    @property
    def related_role_ids(self):
        r"""Gets the related_role_ids of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 关联角色ID列表。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。

        :return: The related_role_ids of this ProtectedBranchProtectedActionBodyDto.
        :rtype: list[str]
        """
        return self._related_role_ids

    @related_role_ids.setter
    def related_role_ids(self, related_role_ids):
        r"""Sets the related_role_ids of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 关联角色ID列表。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。

        :param related_role_ids: The related_role_ids of this ProtectedBranchProtectedActionBodyDto.
        :type related_role_ids: list[str]
        """
        self._related_role_ids = related_role_ids

    @property
    def action(self):
        r"""Gets the action of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 事件名称。 **约束限制：** action中必传此字段。 **取值范围：** - push，推送 。 - merge，合并 。 **默认取值：** 不涉及。

        :return: The action of this ProtectedBranchProtectedActionBodyDto.
        :rtype: str
        """
        return self._action

    @action.setter
    def action(self, action):
        r"""Sets the action of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 事件名称。 **约束限制：** action中必传此字段。 **取值范围：** - push，推送 。 - merge，合并 。 **默认取值：** 不涉及。

        :param action: The action of this ProtectedBranchProtectedActionBodyDto.
        :type action: str
        """
        self._action = action

    @property
    def addition_switchers(self):
        r"""Gets the addition_switchers of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 附加开关。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。

        :return: The addition_switchers of this ProtectedBranchProtectedActionBodyDto.
        :rtype: list[:class:`huaweicloudsdkcodeartsrepo.v4.ProtectedActionSwitcher`]
        """
        return self._addition_switchers

    @addition_switchers.setter
    def addition_switchers(self, addition_switchers):
        r"""Sets the addition_switchers of this ProtectedBranchProtectedActionBodyDto.

        **参数解释：** 附加开关。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。

        :param addition_switchers: The addition_switchers of this ProtectedBranchProtectedActionBodyDto.
        :type addition_switchers: list[:class:`huaweicloudsdkcodeartsrepo.v4.ProtectedActionSwitcher`]
        """
        self._addition_switchers = addition_switchers

    def to_dict(self):
        result = {}

        for attr, _ in self.openapi_types.items():
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                if attr in self.sensitive_list:
                    result[attr] = "****"
                else:
                    result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        import simplejson as json
        return json.dumps(sanitize_for_serialization(self), ensure_ascii=False)

    def __repr__(self):
        """For `print`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, ProtectedBranchProtectedActionBodyDto):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
