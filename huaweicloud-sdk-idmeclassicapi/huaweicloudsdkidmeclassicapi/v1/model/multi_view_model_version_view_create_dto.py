...
class MultiViewModelVersionViewCreateDTO:
    ...
    def __init__(self, version_id=None, item=None, work_copy_type=None, custom_link_set=None, need_set_null=None, modifier=None):
        r"""MultiViewModelVersionViewCreateDTO

        The model defined in huaweicloud sdk

        :param version_id: **参数解释：**  版本对象ID，用于指定待创建视图的基础版本实例。  **约束限制：**  不涉及。  **取值范围：**  不涉及。  **默认取值：**  不涉及。
        :type version_id: str
        :param item: 
        :type item: :class:`huaweicloudsdkidmeclassicapi.v1.ObjectReferenceParamDTO`
        :param work_copy_type: **参数解释：**  关系的复制类型，用于指定创建多维视图后是否继承原实例的关系实例。  **约束限制：**  不涉及。  **取值范围：**  - BOTH：若存在关系实例引用此数据实例作为源端实例或目标端实例，创建多维版本后的数据实例将继承这些关系实例。 - SOURCE：若存在关系实例引用此数据实例作为源端实例，创建多维版本后的数据实例将继承这些关系实例。 - TARGET：若存在关系实例引用此数据实例作为目标端实例，创建多维版本后的数据实例将继承这些关系实例。 - NONE：创建多维版本后的数据实例将不继承任何关系实例。 - CUSTOM：若指定的关系实体集合对应的关系实例引用此数据实例作为源端实例或目标端实例，创建多维版本后的数据实例将继承这些关系实例。  **默认取值：**  不涉及。
        :type work_copy_type: str
        :param custom_link_set: **参数解释：**  关系实体名称集合，与workCopyType的值CUSTOM配合使用。  **约束限制：**  仅当workCopyType为CUSTOM时生效，否则将被忽略。  **取值范围：**  不涉及。  **默认取值：**  不涉及。
        :type custom_link_set: list[str]
        :param need_set_null: **参数解释：**  指定不复制的属性名称列表，其值将被设置为null。  **约束限制：**  不涉及。  **取值范围：**  不涉及。  **默认取值：**  不涉及。
        :type need_set_null: list[str]
        :param modifier: **参数解释：**  更新者账号，用于记录执行创建多维视图操作的用户标识。  **约束限制：**  不涉及。  **取值范围：**  不涉及。  **默认取值：**  不涉及。
        :type modifier: str
        """
