...
class QueryCondition:
    ...
    def __init__(self, condition_name=None, condition_value=None, condition_values=None, conditions=None, ignore_str=None, join_table_alias=None, joiner=None, operator=None, pre_condition=None):
        ...
        :param condition_values: **参数解释：**  查询条件的值。  **约束限制：**  不涉及。  **取值范围：**  不涉及。  **默认取值：**  不涉及。
        :type condition_values: list[str]
        :param conditions: **参数解释：**  查询条件列表。  **约束限制：**  不涉及。  **取值范围：**  - 非多值约束属性：operator为in时有多个值，operator为其他操作符时均为单个值。 - 多值约束属性：operator为contains或&#x3D;时，取值为多个值。operator为其他操作符时均为单个值。  **默认取值：**  不涉及。
        ...
        :type joiner: str
        :param operator: **参数解释：**  操作符。  **约束限制：**  - “多值”为“是”的属性支持使用contains、&#x3D;、like、customLike、startWith、endWith、ISNULL。 - “多值”为“否”的属性不支持使用contains。  **取值范围：**  - &#x3D;：等于查询。 - &lt;：小于查询。 - \\&gt;：大于查询。 - \\&gt;&#x3D;：大于等于查询。 - &lt;&#x3D;：小于等于查询。 - &lt;&gt;：不等于查询。 - startWith：头匹配查询。 - endWith：尾匹配查询。 - like：模糊查询。 - customLike：支持输入*或%的模糊查询。 - in：包含查询。 - not in：排除查询。 - ISNULL：值为空查询。 - NOTNULL：值不为空查询。 - contains：包含查询。  **默认取值：**  不涉及。
        :type operator: str
        :param pre_condition: 
