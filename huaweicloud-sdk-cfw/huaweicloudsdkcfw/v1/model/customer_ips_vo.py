...
class CustomerIpsVO:
    ...
    def __init__(self, action=None, group_id=None, affected_os=None, attack_type=None, config_status=None, contents=None, direction=None, dst_port=None, ips_cfw_id=None, ips_id=None, ips_name=None, protocol=None, severity=None, software=None, src_port=None):
        ...
        :type action: int
        :param group_id: **参数解释**： 防火墙集群id，参数已废弃 **约束限制**： 不涉及 **取值范围**： 不涉及 **默认取值**：   不涉及
        :type group_id: str
        ...
        :param config_status: **参数解释**： 规则状态 **约束限制**： 不涉及 **取值范围**： 0：初始化 1：配置中 2：配置成功 3：配置失败 **默认取值**：   不涉及
        :type config_status: int
        :param contents: **参数解释**： 匹配IPS攻击的报文内容 **约束限制**： 不涉及 **取值范围**： **默认取值**：   不涉及
        :type contents: list[:class:`huaweicloudsdkcfw.v1.IpsContent`]
        :param direction: **参数解释**： 方向 **约束限制**： 不涉及 **取值范围**： 0：客户端到服务端 1：服务端到客户端 **默认取值**：   不涉及
        :type direction: int
        :param dst_port: 
