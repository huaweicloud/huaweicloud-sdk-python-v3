...
class ClientNetworkRequestBody:
    ...
    def __init__(self, client_network_ranges=None):
        r"""ClientNetworkRequestBody

        The model defined in huaweicloud sdk

        :param client_network_ranges: 客户端所在网段。 跨网段访问配置只有在客户端与副本集实例部署在不同网段的情况下才需要配置，例如访问副本集的客户端所在网段为192.168.0.0/16，副本集所在的网段为172.16.0.0/24，则需要添加跨网段配置192.168.0.0/16才能正常访问。只有副本集有该功能。 注意：源端ECS连接实例的前提是与实例节点网络通信正常，如果网络不通，可以参考对等连接进行相关配置。
    ...
    def client_network_ranges(self):
        r"""Gets the client_network_ranges of this ClientNetworkRequestBody.
        ...
        :return: The client_network_ranges of this ClientNetworkRequestBody.
        :rtype: list[str]
        """
    ...
    def client_network_ranges(self, client_network_ranges):
        r"""Sets the client_network_ranges of this ClientNetworkRequestBody.

        客户端所在网段。 跨网段访问配置只有在客户端与副本集实例部署在不同网段的情况下才需要配置，例如访问副本集的客户端所在网段为192.168.0.0/16，副本集所在的网段为172.16.0.0/24，则需要添加跨网段配置192.168.0.0/16才能正常访问。只有副本集有该功能。 注意：源端ECS连接实例的前提是与实例节点网络通信正常，如果网络不通，可以参考对等连接进行相关配置。

        :param client_network_ranges: The client_network_ranges of this ClientNetworkRequestBody.
        :type client_network_ranges: list[str]
        """
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ClientNetworkRequestBody):
            return False
