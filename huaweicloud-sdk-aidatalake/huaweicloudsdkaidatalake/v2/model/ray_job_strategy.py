...
class RayJobStrategy:
    ...
    def __init__(self, queued_timeout=None, running_timeout=None, max_retry=None, retry_delay=None):
        r"""RayJobStrategy

        The model defined in huaweicloud sdk

        :param queued_timeout: 作业提交超时时间，如果超过此时间作业仍未运行，则作业失败。默认作业提交超时时间为3小时，最大值为1天，最小值为10分钟。单位：分钟。取值范围为10~1440，单位为分钟。
        :type queued_timeout: int
        :param running_timeout: 作业运行超时时间，如果超过此时间作业还未运行结束，则作业会取消运行，并标记失败。默认值为-1，表示没有配置作业运行超时时间。最小时间为10分钟，最大时间为7天。单位：分钟。取值范围为-1~10080（-1表示不配置超时），单位为分钟。
    ...
    def queued_timeout(self):
        r"""Gets the queued_timeout of this RayJobStrategy.

        作业提交超时时间，如果超过此时间作业仍未运行，则作业失败。默认作业提交超时时间为3小时，最大值为1天，最小值为10分钟。单位：分钟。取值范围为10~1440，单位为分钟。

        :return: The queued_timeout of this RayJobStrategy.
        :rtype: int
        """
    ...
    def queued_timeout(self, queued_timeout):
        r"""Sets the queued_timeout of this RayJobStrategy.

        作业提交超时时间，如果超过此时间作业仍未运行，则作业失败。默认作业提交超时时间为3小时，最大值为1天，最小值为10分钟。单位：分钟。取值范围为10~1440，单位为分钟。

        :param queued_timeout: The queued_timeout of this RayJobStrategy.
        :type queued_timeout: int
        """
    ...
    def running_timeout(self, running_timeout):
        r"""Sets the running_timeout of this RayJobStrategy.

        作业运行超时时间，如果超过此时间作业还未运行结束，则作业会取消运行，并标记失败。默认值为-1，表示没有配置作业运行超时时间。最小时间为10分钟，最大时间为7天。单位：分钟。取值范围为-1~10080（-1表示不配置超时），单位为分钟。

        :param running_timeout: The running_timeout of this RayJobStrategy.
        :type running_timeout: int
        """
    ...
    def max_retry(self, max_retry):
        r"""Sets the max_retry of this RayJobStrategy.

        **参数解释**：最大重试次数，用于控制作业失败后的自动重试次数。如果配置的值大于0，则自动开启失败重试；如果不配置或者配置为0，则不开启作业失败重试。 **约束限制**：不涉及。 **取值范围**：0~65535。 **默认取值**：0。

        :param max_retry: The max_retry of this RayJobStrategy.
        :type max_retry: int
        """
    ...
    def retry_delay(self):
        r"""Gets the retry_delay of this RayJobStrategy.

        **参数解释**：重试间隔时间，用于指定作业失败重试机制中每次重试的时间间隔，单位为秒。 **约束限制**：不涉及。 **取值范围**：1~3600。 **默认取值**：30。

        :return: The retry_delay of this RayJobStrategy.
        :rtype: int
        """
    ...
    def retry_delay(self, retry_delay):
        r"""Sets the retry_delay of this RayJobStrategy.

        **参数解释**：重试间隔时间，用于指定作业失败重试机制中每次重试的时间间隔，单位为秒。 **约束限制**：不涉及。 **取值范围**：1~3600。 **默认取值**：30。

        :param retry_delay: The retry_delay of this RayJobStrategy.
        :type retry_delay: int
        """
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, RayJobStrategy):
            return False
