...
class UpdateSubTemplate:
    ...
    @property
    def sub_type(self):
        r"""Gets the sub_type of this UpdateSubTemplate.

        模板子类型，只支持以下类型：sms，dingding（钉钉），wechat（微信），webhook，email

        :return: The sub_type of this UpdateSubTemplate.
        :rtype: str
        """
    ...
    def sub_type(self, sub_type):
        r"""Sets the sub_type of this UpdateSubTemplate.

        模板子类型，只支持以下类型：sms，dingding（钉钉），wechat（微信），webhook，email

        :param sub_type: The sub_type of this UpdateSubTemplate.
        :type sub_type: str
        """
    ...
    @property
    def content(self):
        r"""Gets the content of this UpdateSubTemplate.

        子模板正文，$符号后所跟变量仅支持以下变量，根据不同告警类型（关键词告警和sql告警），所支持的变量亦不相同。目前两种告警类型有共同变量如下：  告警级别：${event_severity}; 发生时间：${starts_at}; 告警源：$event.metadata.resource_provider; 资源类型：$event.metadata.resource_type; 资源标识：${resources}; 统计类型：关键词统计; 表达式：$event.annotations.condition_expression; 当前值: $event.annotations.current_value; 统计周期：$event.annotations.frequency; 关键词告警特有变量： 查询时间：$event.annotations.results[0].time; 查询日志：$event.annotations.results[0].raw_results; sql告警特有变量： 日志组/流名称：$event.annotations.results[0].resource_id; 查询语句：$event.annotations.results[0].sql; 查询时间：$event.annotations.results[0].time; 查询URL：$event.annotations.results[0].url; 查询日志：$event.annotations.results[0].raw_results;   >变量后面的分号\";\"为英文符号，必须添加，否则模板会出现替换失败的情况。

        :return: The content of this UpdateSubTemplate.
        :rtype: str
        """
    ...
    def content(self, content):
        r"""Sets the content of this UpdateSubTemplate.

        子模板正文，$符号后所跟变量仅支持以下变量，根据不同告警类型（关键词告警和sql告警），所支持的变量亦不相同。目前两种告警类型有共同变量如下：  告警级别：${event_severity}; 发生时间：${starts_at}; 告警源：$event.metadata.resource_provider; 资源类型：$event.metadata.resource_type; 资源标识：${resources}; 统计类型：关键词统计; 表达式：$event.annotations.condition_expression; 当前值: $event.annotations.current_value; 统计周期：$event.annotations.frequency; 关键词告警特有变量： 查询时间：$event.annotations.results[0].time; 查询日志：$event.annotations.results[0].raw_results; sql告警特有变量： 日志组/流名称：$event.annotations.results[0].resource_id; 查询语句：$event.annotations.results[0].sql; 查询时间：$event.annotations.results[0].time; 查询URL：$event.annotations.results[0].url; 查询日志：$event.annotations.results[0].raw_results;   >变量后面的分号\";\"为英文符号，必须添加，否则模板会出现替换失败的情况。

        :param content: The content of this UpdateSubTemplate.
        :type content: str
        """
    ...
    @property
    def topic(self):
        r"""Gets the topic of this UpdateSubTemplate.

        邮件主题,只有sub_type=email时生效

        :return: The topic of this UpdateSubTemplate.
        :rtype: str
        """
        return self._topic
    ...
    def topic(self, topic):
        r"""Sets the topic of this UpdateSubTemplate.

        邮件主题,只有sub_type=email时生效

        :param topic: The topic of this UpdateSubTemplate.
        :type topic: str
        """
        self._topic = topic
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, UpdateSubTemplate):
            return False
