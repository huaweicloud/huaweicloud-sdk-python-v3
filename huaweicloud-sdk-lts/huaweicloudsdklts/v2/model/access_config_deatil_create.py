...
class AccessConfigDeatilCreate:
    ...
    def __init__(self, paths=None, black_paths=None, format=None, windows_log_info=None, stdout=None, stderr=None, combine_stdout=None, path_type=None, namespace_regex=None, pod_name_regex=None, container_name_regex=None, include_labels=None, exclude_labels=None, include_envs=None, exclude_envs=None, log_labels=None, log_envs=None, include_k8s_labels=None, exclude_k8s_labels=None, log_k8s=None, repeat_collect=None, system_fields=None, custom_key_value=None, include_labels_logical=None, exclude_labels_logical=None, include_k8s_labels_logical=None, exclude_k8s_labels_logical=None, include_envs_logical=None, exclude_envs_logical=None):
    ...
    def combine_stdout(self):
        r"""Gets the combine_stdout of this AccessConfigDeatilCreate.
        ...
        **参数解释：** 云容器引擎CCE应用日志接入时，是否将标准输出和标准错误采集到同一个文件。 **取值范围：** - true：将标准输出和标准错误采集到同一个文件（stdout.log）。 - false：将标准输出和标准错误采集到不同的文件（stdout.log和stderr.log）。
        ...
        :return: The combine_stdout of this AccessConfigDeatilCreate.
        :rtype: bool
        """
    ...
    def combine_stdout(self, combine_stdout):
        r"""Sets the combine_stdout of this AccessConfigDeatilCreate.

        **参数解释：** 云容器引擎CCE应用日志接入时，是否将标准输出和标准错误采集到同一个文件。 **取值范围：** - true：将标准输出和标准错误采集到同一个文件（stdout.log）。 - false：将标准输出和标准错误采集到不同的文件（stdout.log和stderr.log）。

        :param combine_stdout: The combine_stdout of this AccessConfigDeatilCreate.
        :type combine_stdout: bool
        """
