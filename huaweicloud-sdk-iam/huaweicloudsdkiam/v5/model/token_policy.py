...
class TokenPolicy:
    ...
    def token_enabled(self, token_enabled):
        r"""Sets the token_enabled of this TokenPolicy.

        是否允许获取Token，默认为true，设置为false后将不允许获取账号下所有身份类型（IAM用户、委托、联邦用户）的Token（联邦认证获取的unscoped token不受Token策略影响）。

        :param token_enabled: The token_enabled of this TokenPolicy.
        :type token_enabled: bool
        """
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, TokenPolicy):
            return False
