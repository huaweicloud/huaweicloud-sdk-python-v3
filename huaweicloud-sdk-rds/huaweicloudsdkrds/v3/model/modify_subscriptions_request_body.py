# coding: utf-8

from huaweicloudsdkcore.utils.http_utils import sanitize_for_serialization


class ModifySubscriptionsRequestBody:

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    sensitive_list = []

    openapi_types = {
        'subscription_ids': 'list[str]',
        'job_schedule': 'OperateUsedJobSchedule'
    }

    attribute_map = {
        'subscription_ids': 'subscription_ids',
        'job_schedule': 'job_schedule'
    }

    def __init__(self, subscription_ids=None, job_schedule=None):
        r"""ModifySubscriptionsRequestBody

        The model defined in huaweicloud sdk

        :param subscription_ids: 修改的订阅id列表。每次修改的订阅必须属于同一实例。
        :type subscription_ids: list[str]
        :param job_schedule: 
        :type job_schedule: :class:`huaweicloudsdkrds.v3.OperateUsedJobSchedule`
        """
        
        

        self._subscription_ids = None
        self._job_schedule = None
        self.discriminator = None

        self.subscription_ids = subscription_ids
        self.job_schedule = job_schedule

    @property
    def subscription_ids(self):
        r"""Gets the subscription_ids of this ModifySubscriptionsRequestBody.

        修改的订阅id列表。每次修改的订阅必须属于同一实例。

        :return: The subscription_ids of this ModifySubscriptionsRequestBody.
        :rtype: list[str]
        """
        return self._subscription_ids

    @subscription_ids.setter
    def subscription_ids(self, subscription_ids):
        r"""Sets the subscription_ids of this ModifySubscriptionsRequestBody.

        修改的订阅id列表。每次修改的订阅必须属于同一实例。

        :param subscription_ids: The subscription_ids of this ModifySubscriptionsRequestBody.
        :type subscription_ids: list[str]
        """
        self._subscription_ids = subscription_ids

    @property
    def job_schedule(self):
        r"""Gets the job_schedule of this ModifySubscriptionsRequestBody.

        :return: The job_schedule of this ModifySubscriptionsRequestBody.
        :rtype: :class:`huaweicloudsdkrds.v3.OperateUsedJobSchedule`
        """
        return self._job_schedule

    @job_schedule.setter
    def job_schedule(self, job_schedule):
        r"""Sets the job_schedule of this ModifySubscriptionsRequestBody.

        :param job_schedule: The job_schedule of this ModifySubscriptionsRequestBody.
        :type job_schedule: :class:`huaweicloudsdkrds.v3.OperateUsedJobSchedule`
        """
        self._job_schedule = job_schedule

    def to_dict(self):
        result = {}

        for attr, _ in self.openapi_types.items():
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                if attr in self.sensitive_list:
                    result[attr] = "****"
                else:
                    result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        import simplejson as json
        return json.dumps(sanitize_for_serialization(self), ensure_ascii=False)

    def __repr__(self):
        """For `print`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, ModifySubscriptionsRequestBody):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
