# coding: utf-8

from huaweicloudsdkcore.utils.http_utils import sanitize_for_serialization


class MemorySearchResp:

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    sensitive_list = []

    openapi_types = {
        'results': 'list[MemorySearchRespResults]',
        'query': 'str',
        'total': 'int'
    }

    attribute_map = {
        'results': 'results',
        'query': 'query',
        'total': 'total'
    }

    def __init__(self, results=None, query=None, total=None):
        r"""MemorySearchResp

        The model defined in huaweicloud sdk

        :param results: **参数解释：** 检索结果（按 Rerank 分数降序）。 **约束限制：** 不涉及。 **取值范围：** 0-100个。 **默认取值：** 不涉及。 
        :type results: list[:class:`huaweicloudsdkagentarts.v1.MemorySearchRespResults`]
        :param query: **参数解释：**  原始查询文本，与请求中的 query 参数一致。 **约束限制：**  不涉及。 **取值范围：** 长度0-2000字符。 **默认取值：** 0-2000 字符。 
        :type query: str
        :param total: **参数解释：**  符合条件的搜索结果总数，用于计算总页数。 **约束限制：**  不涉及。 **取值范围：**  0-2147483647 **默认取值：**  0-2147483647 
        :type total: int
        """
        
        

        self._results = None
        self._query = None
        self._total = None
        self.discriminator = None

        self.results = results
        if query is not None:
            self.query = query
        self.total = total

    @property
    def results(self):
        r"""Gets the results of this MemorySearchResp.

        **参数解释：** 检索结果（按 Rerank 分数降序）。 **约束限制：** 不涉及。 **取值范围：** 0-100个。 **默认取值：** 不涉及。 

        :return: The results of this MemorySearchResp.
        :rtype: list[:class:`huaweicloudsdkagentarts.v1.MemorySearchRespResults`]
        """
        return self._results

    @results.setter
    def results(self, results):
        r"""Sets the results of this MemorySearchResp.

        **参数解释：** 检索结果（按 Rerank 分数降序）。 **约束限制：** 不涉及。 **取值范围：** 0-100个。 **默认取值：** 不涉及。 

        :param results: The results of this MemorySearchResp.
        :type results: list[:class:`huaweicloudsdkagentarts.v1.MemorySearchRespResults`]
        """
        self._results = results

    @property
    def query(self):
        r"""Gets the query of this MemorySearchResp.

        **参数解释：**  原始查询文本，与请求中的 query 参数一致。 **约束限制：**  不涉及。 **取值范围：** 长度0-2000字符。 **默认取值：** 0-2000 字符。 

        :return: The query of this MemorySearchResp.
        :rtype: str
        """
        return self._query

    @query.setter
    def query(self, query):
        r"""Sets the query of this MemorySearchResp.

        **参数解释：**  原始查询文本，与请求中的 query 参数一致。 **约束限制：**  不涉及。 **取值范围：** 长度0-2000字符。 **默认取值：** 0-2000 字符。 

        :param query: The query of this MemorySearchResp.
        :type query: str
        """
        self._query = query

    @property
    def total(self):
        r"""Gets the total of this MemorySearchResp.

        **参数解释：**  符合条件的搜索结果总数，用于计算总页数。 **约束限制：**  不涉及。 **取值范围：**  0-2147483647 **默认取值：**  0-2147483647 

        :return: The total of this MemorySearchResp.
        :rtype: int
        """
        return self._total

    @total.setter
    def total(self, total):
        r"""Sets the total of this MemorySearchResp.

        **参数解释：**  符合条件的搜索结果总数，用于计算总页数。 **约束限制：**  不涉及。 **取值范围：**  0-2147483647 **默认取值：**  0-2147483647 

        :param total: The total of this MemorySearchResp.
        :type total: int
        """
        self._total = total

    def to_dict(self):
        result = {}

        for attr, _ in self.openapi_types.items():
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                if attr in self.sensitive_list:
                    result[attr] = "****"
                else:
                    result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        import simplejson as json
        return json.dumps(sanitize_for_serialization(self), ensure_ascii=False)

    def __repr__(self):
        """For `print`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, MemorySearchResp):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
