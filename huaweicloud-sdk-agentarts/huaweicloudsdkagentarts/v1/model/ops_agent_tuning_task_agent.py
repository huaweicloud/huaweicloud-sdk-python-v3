...
class OpsAgentTuningTaskAgent:
    ...
    @type.setter
    def type(self, type):
        r"""Sets the type of this OpsAgentTuningTaskAgent.

        **参数解释：** 智能体类型。  **约束限制：** 不涉及  **取值范围：** agent：单智能体，workflow：工作流，agentrun：部署在运行时的智能体，third_party：三方接入观测的智能体。  **默认取值：** 无

        :param type: The type of this OpsAgentTuningTaskAgent.
        :type type: str
        """
        self._type = type
    ...
    @id.setter
    def id(self, id):
        r"""Sets the id of this OpsAgentTuningTaskAgent.

        **参数解释：** 智能体ID，对应AgentArts平台中的智能体实例。 可在AgentArts平台“智能体管理”页面获取ID，注意对应的智能体需要是“已提交”状态。  **约束限制：** 必须是系统中存在的真实ID。  **取值范围：** 有效标识符字符串。  **默认取值：** 无

        :param id: The id of this OpsAgentTuningTaskAgent.
        :type id: str
        """
        self._id = id
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, OpsAgentTuningTaskAgent):
            return False
