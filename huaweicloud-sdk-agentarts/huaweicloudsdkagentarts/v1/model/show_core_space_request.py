...
class ShowCoreSpaceRequest:
    ...
    @property
    def space_id(self):
        r"""Gets the space_id of this ShowCoreSpaceRequest.
        ...
        **参数解释：**  记忆库 ID，唯一标识一个记忆库，可以通过AgentArts智能体记忆库控制台或者记忆库查询接口获取。 **约束限制：** 仅支持字母、数字和中划线。 **取值范围：** 匹配标准的UUID格式（8-4-4-4-12的十六进制数字串，由连字符分隔），符合正则条件^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$。 **默认取值：** 不涉及。 
        ...
        :return: The space_id of this ShowCoreSpaceRequest.
        :rtype: str
        """
    ...
    def space_id(self, space_id):
        r"""Sets the space_id of this ShowCoreSpaceRequest.

        **参数解释：**  记忆库 ID，唯一标识一个记忆库，可以通过AgentArts智能体记忆库控制台或者记忆库查询接口获取。 **约束限制：** 仅支持字母、数字和中划线。 **取值范围：** 匹配标准的UUID格式（8-4-4-4-12的十六进制数字串，由连字符分隔），符合正则条件^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$。 **默认取值：** 不涉及。 

        :param space_id: The space_id of this ShowCoreSpaceRequest.
        :type space_id: str
        """
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ShowCoreSpaceRequest):
            return False
