...
class ListCoreGatewaysByTagsRequestBody:
    ...
    def __init__(self, without_any_tag=None, tags=None, matches=None):
        r"""ListCoreGatewaysByTagsRequestBody
    ...
    def without_any_tag(self):
        r"""Gets the without_any_tag of this ListCoreGatewaysByTagsRequestBody.

        **参数解释：** 是否查询所有不带标签的资源。 **约束限制：** 不涉及。 **取值范围：** - true：查询所有不带标签的资源，此时忽略 “tags”字段； - false：返回所有资源或按“tags”、“matches”等条件过滤。 **默认取值：** false。 

        :return: The without_any_tag of this ListCoreGatewaysByTagsRequestBody.
        :rtype: bool
        """
    ...
    def without_any_tag(self, without_any_tag):
        r"""Sets the without_any_tag of this ListCoreGatewaysByTagsRequestBody.

        **参数解释：** 是否查询所有不带标签的资源。 **约束限制：** 不涉及。 **取值范围：** - true：查询所有不带标签的资源，此时忽略 “tags”字段； - false：返回所有资源或按“tags”、“matches”等条件过滤。 **默认取值：** false。 

        :param without_any_tag: The without_any_tag of this ListCoreGatewaysByTagsRequestBody.
        :type without_any_tag: bool
        """
    ...
    @property
    def tags(self):
        r"""Gets the tags of this ListCoreGatewaysByTagsRequestBody.
        ...
        **参数解释：** 指定查询的资源中需要包含哪些标签，结果返回包含所有标签的资源列表，key之间是与的关系，key-value结构中value是或的关系。 **约束限制：** 数组内key不能重复，同一个key中values不能重复，每个key对应的value可以为空数组但结构体不能缺失。 **取值范围：** 数组长度最大为20。 **默认取值：** 不涉及。 
        ...
        :return: The tags of this ListCoreGatewaysByTagsRequestBody.
    ...
    @tags.setter
    def tags(self, tags):
        r"""Sets the tags of this ListCoreGatewaysByTagsRequestBody.

        **参数解释：** 指定查询的资源中需要包含哪些标签，结果返回包含所有标签的资源列表，key之间是与的关系，key-value结构中value是或的关系。 **约束限制：** 数组内key不能重复，同一个key中values不能重复，每个key对应的value可以为空数组但结构体不能缺失。 **取值范围：** 数组长度最大为20。 **默认取值：** 不涉及。 

        :param tags: The tags of this ListCoreGatewaysByTagsRequestBody.
        :type tags: list[:class:`huaweicloudsdkagentarts.v1.CoreGatewaysTagValuesForTMS`]
        """
        self._tags = tags
    ...
    @property
    def matches(self):
        r"""Gets the matches of this ListCoreGatewaysByTagsRequestBody.

        **参数解释：** 标签键值匹配字段，key为要匹配的字段。 **约束限制：** key为固定字典值，只支持resource_name，不能包含重复的key或不支持的key。支持根据value的值做前缀搜索。 **取值范围：** 数组长度最大为1。 **默认取值：** 不涉及。 

        :return: The matches of this ListCoreGatewaysByTagsRequestBody.
    ...
    def matches(self, matches):
        r"""Sets the matches of this ListCoreGatewaysByTagsRequestBody.

        **参数解释：** 标签键值匹配字段，key为要匹配的字段。 **约束限制：** key为固定字典值，只支持resource_name，不能包含重复的key或不支持的key。支持根据value的值做前缀搜索。 **取值范围：** 数组长度最大为1。 **默认取值：** 不涉及。 

        :param matches: The matches of this ListCoreGatewaysByTagsRequestBody.
        :type matches: list[:class:`huaweicloudsdkagentarts.v1.CoreGatewaysMatchForTMS`]
        """
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ListCoreGatewaysByTagsRequestBody):
            return False
