# coding: utf-8

from huaweicloudsdkcore.sdk_response import SdkResponse
from huaweicloudsdkcore.utils.http_utils import sanitize_for_serialization


class ListDdmEnginesResponse(SdkResponse):

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    sensitive_list = []

    openapi_types = {
        'engine_groups': 'list[EngineGroupInfo]',
        'offset': 'int',
        'limit': 'int',
        'total': 'int'
    }

    attribute_map = {
        'engine_groups': 'engine_groups',
        'offset': 'offset',
        'limit': 'limit',
        'total': 'total'
    }

    def __init__(self, engine_groups=None, offset=None, limit=None, total=None):
        r"""ListDdmEnginesResponse

        The model defined in huaweicloud sdk

        :param engine_groups: 引擎信息列表。
        :type engine_groups: list[:class:`huaweicloudsdkddm.v1.EngineGroupInfo`]
        :param offset: 分页参数: 起始值。
        :type offset: int
        :param limit: 分页参数：每页多少条。
        :type limit: int
        :param total: 引擎信息总数。
        :type total: int
        """
        
        super().__init__()

        self._engine_groups = None
        self._offset = None
        self._limit = None
        self._total = None
        self.discriminator = None

        if engine_groups is not None:
            self.engine_groups = engine_groups
        if offset is not None:
            self.offset = offset
        if limit is not None:
            self.limit = limit
        if total is not None:
            self.total = total

    @property
    def engine_groups(self):
        r"""Gets the engine_groups of this ListDdmEnginesResponse.

        引擎信息列表。

        :return: The engine_groups of this ListDdmEnginesResponse.
        :rtype: list[:class:`huaweicloudsdkddm.v1.EngineGroupInfo`]
        """
        return self._engine_groups

    @engine_groups.setter
    def engine_groups(self, engine_groups):
        r"""Sets the engine_groups of this ListDdmEnginesResponse.

        引擎信息列表。

        :param engine_groups: The engine_groups of this ListDdmEnginesResponse.
        :type engine_groups: list[:class:`huaweicloudsdkddm.v1.EngineGroupInfo`]
        """
        self._engine_groups = engine_groups

    @property
    def offset(self):
        r"""Gets the offset of this ListDdmEnginesResponse.

        分页参数: 起始值。

        :return: The offset of this ListDdmEnginesResponse.
        :rtype: int
        """
        return self._offset

    @offset.setter
    def offset(self, offset):
        r"""Sets the offset of this ListDdmEnginesResponse.

        分页参数: 起始值。

        :param offset: The offset of this ListDdmEnginesResponse.
        :type offset: int
        """
        self._offset = offset

    @property
    def limit(self):
        r"""Gets the limit of this ListDdmEnginesResponse.

        分页参数：每页多少条。

        :return: The limit of this ListDdmEnginesResponse.
        :rtype: int
        """
        return self._limit

    @limit.setter
    def limit(self, limit):
        r"""Sets the limit of this ListDdmEnginesResponse.

        分页参数：每页多少条。

        :param limit: The limit of this ListDdmEnginesResponse.
        :type limit: int
        """
        self._limit = limit

    @property
    def total(self):
        r"""Gets the total of this ListDdmEnginesResponse.

        引擎信息总数。

        :return: The total of this ListDdmEnginesResponse.
        :rtype: int
        """
        return self._total

    @total.setter
    def total(self, total):
        r"""Sets the total of this ListDdmEnginesResponse.

        引擎信息总数。

        :param total: The total of this ListDdmEnginesResponse.
        :type total: int
        """
        self._total = total

    def to_dict(self):
        import warnings
        warnings.warn("ListDdmEnginesResponse.to_dict() is deprecated and no longer maintained, "
                      "use to_json_object() to get the response content.", DeprecationWarning)
        result = {}

        for attr, _ in self.openapi_types.items():
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                if attr in self.sensitive_list:
                    result[attr] = "****"
                else:
                    result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        import simplejson as json
        return json.dumps(sanitize_for_serialization(self), ensure_ascii=False)

    def __repr__(self):
        """For `print`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, ListDdmEnginesResponse):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
