...
class ClusterNodeInfo:
    ...
    def __init__(self, id=None, name=None, status=None, sub_status=None, spec=None, inst_create_type=None, alias_name=None, az_code=None):
        ...
        :type name: str
        :param status: **参数解释**： 节点状态。 **取值范围**： - CREATING：创建中。 - FREE：空闲。 - ACTIVE：正常。 - FAILED：不可用。 - CREATE_FAILED：创建失败。 - DELETING：删除中。 - FROZEN：冻结。 - STOPPED：关机。
        :type status: str
        :param sub_status: **参数解释**： 节点子状态。 **取值范围**： - READY：ECS已就绪。 - PREPARED：软件已部署。 - INITED：节点已正常。 - CREATED：节点已监控。
        :type sub_status: str
