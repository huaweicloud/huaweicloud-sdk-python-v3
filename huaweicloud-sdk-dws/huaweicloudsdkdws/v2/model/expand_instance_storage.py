...
class ExpandInstanceStorage:
    ...
    def __init__(self, new_size=None):
        ...
        :param new_size: **参数解释**： 磁盘扩容后单节点有效存储容量（GB/节点）。 该容量必须大于当前单节点有效容量，小于等于集群规格支持的单节点最大容量，扩容容量为规格支持的步长倍数。 集群规格配置详情可根据“查询规格信息”查询。  **约束限制**： 不涉及。  **取值范围**： 不涉及。  **默认取值**： 不涉及。
        :type new_size: int
        """
    ...
    def new_size(self):
        ...
        **参数解释**： 磁盘扩容后单节点有效存储容量（GB/节点）。 该容量必须大于当前单节点有效容量，小于等于集群规格支持的单节点最大容量，扩容容量为规格支持的步长倍数。 集群规格配置详情可根据“查询规格信息”查询。  **约束限制**： 不涉及。  **取值范围**： 不涉及。  **默认取值**： 不涉及。
