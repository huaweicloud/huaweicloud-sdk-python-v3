...
class RemoveSecurityGroupsRequestBody:
    ...
    def __init__(self, port=None, dry_run=None):
        ...
        :param dry_run: 功能说明：是否只预检此次请求 取值范围： -true：发送检查请求，不会执行端口移除安全组。检查项包括是否填写了必需参数、请求格式、业务限制。如果检查不通过，则返回对应错误。如果检查通过，则返回响应码202。 -false（默认值）：发送正常请求，并直接执行端口移除安全组。
        :type dry_run: bool
        """
    ...
    @property
    def dry_run(self):
        r"""Gets the dry_run of this RemoveSecurityGroupsRequestBody.
        ...
        :return: The dry_run of this RemoveSecurityGroupsRequestBody.
        :rtype: bool
        """
    ...
    def dry_run(self, dry_run):
        r"""Sets the dry_run of this RemoveSecurityGroupsRequestBody.

        功能说明：是否只预检此次请求 取值范围： -true：发送检查请求，不会执行端口移除安全组。检查项包括是否填写了必需参数、请求格式、业务限制。如果检查不通过，则返回对应错误。如果检查通过，则返回响应码202。 -false（默认值）：发送正常请求，并直接执行端口移除安全组。

        :param dry_run: The dry_run of this RemoveSecurityGroupsRequestBody.
        :type dry_run: bool
        """
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, RemoveSecurityGroupsRequestBody):
            return False
