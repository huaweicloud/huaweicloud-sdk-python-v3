...
class ShowRedisPitrPolicyResponse(SdkResponse):
    ...
    def __init__(self, enabled=None, interval=None, keep_days=None):
        ...
        :type enabled: bool
        :param interval: **参数解释：** 数据备份的时间间隔，该数据备份控制redis实例可恢复时间点的间隔，仅在开启时返回。 **取值范围：** 不涉及。
        :type interval: int
        :param keep_days: **参数解释：** 指定已生成的备份文件可以保存的天数，仅在开启时返回。 **取值范围：** 不涉及。
        :type keep_days: int
        """
    ...
    def to_dict(self):
        import warnings
        warnings.warn("ShowRedisPitrPolicyResponse.to_dict() is deprecated and no longer maintained, "
