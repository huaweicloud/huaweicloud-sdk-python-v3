...
class ListInstanceConsumerGroupsResponse(SdkResponse):
    ...
    def __init__(self, total=None, groups=None, max=None, remaining=None, next_offset=None, previous_offset=None):
        ...
        :param total: **参数解释**： 消费组总数。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type total: float
        :param groups: **参数解释**： 消费组列表。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type groups: list[:class:`huaweicloudsdkrocketmq.v2.ConsumerGroup`]
        :param max: **参数解释**： 最大可创建消费组数量。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type max: int
        :param remaining: **参数解释**： 剩余可创建消费组数量。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type remaining: int
    ...
    def to_dict(self):
        import warnings
        warnings.warn("ListInstanceConsumerGroupsResponse.to_dict() is deprecated and no longer maintained, "
