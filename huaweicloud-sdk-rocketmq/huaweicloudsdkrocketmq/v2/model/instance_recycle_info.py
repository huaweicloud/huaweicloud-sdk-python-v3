...
class InstanceRecycleInfo:
    ...
    def __init__(self, instance_id=None, status=None, name=None, engine=None, in_recycle_time=None, save_time=None, auto_delete_time=None, cost_per_hour=None, error_message=None, product_id=None):
        ...
        :param status: **参数解释**： 实例状态。  **约束限制**： 不涉及。 **取值范围**： [详细状态说明请参考[实例状态说明](hrm-api-0010.xml)。](tag:hws,hws_hk,ctc,hws_eu,ocb,g42,hk_g42,tm,sbc,hk_sbc,cmcc,hk_tm,dt,cmcc,ax,srg)[详细状态说明请参考[实例状态说明](kafka-api-180514012.xml)。](tag:hcs,fcs,hcs_oemout) **默认取值**： 不涉及。
        :type status: str
        ...
        :type engine: str
        :param in_recycle_time: **参数解释**： 回收时间。  **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type in_recycle_time: int
        :param save_time: **参数解释**： 保留时间。  **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。 **取值范围**： 1~7。
        :type save_time: int
        :param auto_delete_time: **参数解释**： 自动删除时间。  **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type auto_delete_time: int
        :param cost_per_hour: **参数解释**： 每小时的费用。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
