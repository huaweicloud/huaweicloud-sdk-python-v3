...
class ListEngineProductsResponse(SdkResponse):
    ...
    def __init__(self, total=None, next_offset=None, previous_offset=None, engine=None, versions=None, products=None):
        ...
        :param total: **参数解释**： 总数。 **约束限制**： 不涉及。 **取值范围**： 大于等于0。 **默认取值**： 不涉及。
        :type total: float
        ...
        :param engine: **参数解释**： 引擎类型。 **约束限制**： 不涉及。 **取值范围**： - rocketmq：RocketMQ消息引擎。 - reliability：RocketMQ消息引擎别称。 **默认取值**： 不涉及。
        :type engine: str
        :param versions: **参数解释**： 支持的版本。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type versions: list[str]
        :param products: **参数解释**： 产品详情列表。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
    ...
    def to_dict(self):
        import warnings
        warnings.warn("ListEngineProductsResponse.to_dict() is deprecated and no longer maintained, "
