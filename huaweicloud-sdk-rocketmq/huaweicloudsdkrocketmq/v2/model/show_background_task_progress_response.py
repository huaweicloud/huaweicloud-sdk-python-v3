...
class ShowBackgroundTaskProgressResponse(SdkResponse):
    ...
    def __init__(self, progress_percentage=None, remain_time=None, step_list=None):
        ...
        :param progress_percentage: **参数解释**： 后台任务处理进度。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type progress_percentage: int
        :param remain_time: **参数解释**： 剩余时间。 **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type remain_time: int
        :param step_list: **参数解释**： 步骤列表。   **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        :type step_list: list[:class:`huaweicloudsdkrocketmq.v2.StepDetail`]
        """
    ...
    def step_list(self):
        ...
        **参数解释**： 步骤列表。   **约束限制**： 不涉及。 **取值范围**： 不涉及。 **默认取值**： 不涉及。
        ...
        :rtype: list[:class:`huaweicloudsdkrocketmq.v2.StepDetail`]
        """
