...
class BssParam:
    ...
    def __init__(self, is_auto_renew=None, charging_mode=None, is_auto_pay=None, period_type=None, period_num=None):
        ...
        :param charging_mode: **参数解释**： 计费模式。功能说明：付费方式。 **约束限制**： 不涉及。 **取值范围**： - prePaid：预付费，即包年包月； - postPaid：后付费，即按需付费； **默认取值**： postPaid。
        :type charging_mode: str
        :param is_auto_pay: **参数解释**： 下单订购后，是否自动从客户的账户中支付，而不需要客户手动去进行支付。 **约束限制**： 不涉及。 **取值范围**： - true：是（自动支付） - false：否（需要客户手动支付） **默认取值**： 不涉及。
