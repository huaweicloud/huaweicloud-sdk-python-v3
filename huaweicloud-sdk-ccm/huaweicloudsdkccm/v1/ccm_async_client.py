...
class CcmAsyncClient(Client):
    ...
    def new_builder(cls, clazz=None):
        if not clazz:
            ...
            if clazz.__name__ != "CcmAsyncClient":
                raise TypeError("client type error, support client type is CcmAsyncClient")
    ...
    def update_ocsp_switch_async(self, request):
        r"""更新OCSP开关
    ...
    def update_ocsp_switch_async_invoker(self, request):
        http_info = self._update_ocsp_switch_http_info(request)
        return AsyncInvoker(self, http_info)

    def _update_ocsp_switch_http_info(self, request):
        http_info = {
            "method": "POST",
