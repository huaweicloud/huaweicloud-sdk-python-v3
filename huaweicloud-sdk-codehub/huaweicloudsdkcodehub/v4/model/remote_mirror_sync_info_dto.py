...
class RemoteMirrorSyncInfoDto:
    ...
    def __init__(self, username=None, password=None, endpoint_uuid=None, force_fetch=None):
        r"""RemoteMirrorSyncInfoDto

        The model defined in huaweicloud sdk

        :param username: **参数解释：** 用户名(需要base64加密)。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及
        :type username: str
        :param password: **参数解释：** 密码(需要base64加密)。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及
        :type password: str
        :param endpoint_uuid: **参数解释：** 拓展点uuid。 **约束限制：** 不涉及。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
        :type endpoint_uuid: str
        :param force_fetch: **参数解释：** 允许强制同步。 **约束限制：** 不涉及。 **取值范围：** - true，强制同步。 - false，不强制同步。 **默认取值：** 不涉及。
    ...
    def force_fetch(self):
        r"""Gets the force_fetch of this RemoteMirrorSyncInfoDto.

        **参数解释：** 允许强制同步。 **约束限制：** 不涉及。 **取值范围：** - true，强制同步。 - false，不强制同步。 **默认取值：** 不涉及。

        :return: The force_fetch of this RemoteMirrorSyncInfoDto.
        :rtype: bool
        """
    ...
    def force_fetch(self, force_fetch):
        r"""Sets the force_fetch of this RemoteMirrorSyncInfoDto.

        **参数解释：** 允许强制同步。 **约束限制：** 不涉及。 **取值范围：** - true，强制同步。 - false，不强制同步。 **默认取值：** 不涉及。

        :param force_fetch: The force_fetch of this RemoteMirrorSyncInfoDto.
        :type force_fetch: bool
        """
