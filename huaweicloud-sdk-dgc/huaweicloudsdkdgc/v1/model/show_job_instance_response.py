...
class ShowJobInstanceResponse(SdkResponse):
    ...
    def __init__(self, job_name=None, instance_id=None, status=None, plan_time=None, start_time=None, end_time=None, execute_time=None, total=None, nodes=None, instance_type=None, force_success=None, ignore_success=None):
        ...
        :param status: 作业实例状态： - waiting：等待运行 - running：运行中 - success：运行成功 - forceSuccess：强制成功 - ignoreSuccess：忽略失败 - freeze：冻结 - skip-by-depend：跳过 - waiting-confirm：待确认执行 - fail： 运行失败 - running-exception：运行异常 - pause： 暂停 - manual-stop：取消
        :type status: str
    ...
    def to_dict(self):
        import warnings
        warnings.warn("ShowJobInstanceResponse.to_dict() is deprecated and no longer maintained, "
