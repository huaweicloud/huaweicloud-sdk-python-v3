...
class CreateListenerQuicConfigOption:
    ...
    def __init__(self, quic_listener_id=None, enable_quic_upgrade=None):
        ...
        :param quic_listener_id: 监听器关联的QUIC监听器ID。指定的listener id必须已存在，且协议类型为QUIC，不能指定为null，否则与enable_quic_upgrade冲突。  [荷兰region不支持QUIC。](tag:dt)
        :type quic_listener_id: str
        :param enable_quic_upgrade: QUIC升级的开启状态。 True:开启QUIC升级； Flase：关闭QUIC升级（默认）。 开启HTTPS监听器升级QUIC监听器能力。  [荷兰region不支持QUIC。](tag:dt)
