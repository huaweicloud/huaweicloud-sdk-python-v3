...
class CloneListenerOption:
    ...
    def __init__(self, name=None, loadbalancer_id=None, protocol_port=None, port_ranges=None, reuse_pool=None, subnet_mapping_list=None):
        ...
        :param reuse_pool: **参数解释**：新监听器是否复用或复制源监听器的后端服务器组和后端服务器的标识。 - 复用：目的监听器将会直接使用源监听器的后端服务器组。 - 复制：系统将会根据原有配置创建新的后端服务器组，然后将其关联到目的监听器使用。 **约束限制**： - 配置为true时，需要开启后端服务器组多挂实例功能。 - 只在独享型场景复制、同VPC场景可配。 **取值范围**： - true：复用源监听器的后端服务器组。 - false：复制源监听器的后端服务器组。 **默认取值**：false
