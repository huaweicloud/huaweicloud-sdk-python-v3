...
class UpdateL7PolicyOption:
    ...
    def __init__(self, admin_state_up=None, description=None, name=None, redirect_listener_id=None, redirect_pool_id=None, redirect_url_config=None, fixed_response_config=None, redirect_pools_extend_config=None, rules=None, priority=None):
        ...
        :param redirect_pool_id: 转发到pool的ID。当action为REDIRECT_TO_POOL时生效。  使用说明： - redirect_pool_id和redirect_pools_config两个都指定时按redirect_pools_config生效。 - 当action为REDIRECT_TO_POOL时为必选字段，不能更新为空或null。 - 指定的pool不能是listener的default_pool。不能是其他listener的l7policy使用的pool。 - 当action为REDIRECT_TO_LISTENER时，不可指定。
