...
class UpdateL7PolicyOption:
    ...
    def __init__(self, admin_state_up=None, description=None, name=None, redirect_listener_id=None, redirect_pool_id=None, redirect_url_config=None, fixed_response_config=None, rules=None, priority=None, redirect_pools_config=None):
        ...
        :param rules: 转发策略关联的转发规则对象。详细参考表 l7rule字段说明。rules列表中最多含有10个rule规则（若rule中包含conditions字段，一条condition算一个规则），且列表中type为HOST_NAME，PATH，METHOD，SOURCE_IP的rule不能重复，至多指定一条。
        ...
        :type priority: int
        :param redirect_pools_config: 转发到的后端主机组的配置。当action为REDIRECT_TO_POOL时生效。   使用说明：   当action为REDIRECT_TO_POOL时redirect_pool_id和redirect_pools_config必须指定一个，两个都指定时按redirect_pools_config生效。  当action为REDIRECT_TO_LISTENER时，不可指定。  只支持全量覆盖。
