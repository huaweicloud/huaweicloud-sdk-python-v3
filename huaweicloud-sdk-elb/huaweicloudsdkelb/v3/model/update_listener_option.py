...
class UpdateListenerOption:
    ...
    def __init__(self, admin_state_up=None, client_ca_tls_container_ref=None, default_pool_id=None, default_tls_container_ref=None, description=None, http2_enable=None, insert_headers=None, name=None, sni_container_refs=None, tls_ciphers_policy=None, security_policy_id=None, enable_member_retry=None, member_timeout=None, client_timeout=None, keepalive_timeout=None, ipgroup=None, transparent_client_ip_enable=None, enhance_l7policy_enable=None):
        ...
        :param http2_enable: 客户端与LB之间的HTTPS请求的HTTP2功能的开启状态。开启后，可提升客户端与LB间的访问性能，但LB与后端服务器间仍采用HTTP1.X协议。QUIC监听器该字段默认为true，不可修改。  非HTTPS/QUIC协议的监听器该字段无效，无论取值如何都不影响监听器正常运行。
        :type http2_enable: bool
        :param insert_headers: 
        :type insert_headers: :class:`huaweicloudsdkelb.v3.ListenerInsertHeaders`
        :param name: 监听器的名称。
        :type name: str
        :param sni_container_refs: 监听器使用的SNI证书（带域名的服务器证书）ID列表。  使用说明： - 列表对应的所有SNI证书的域名不允许存在重复。 - 列表对应的所有SNI证书的域名总数不超过30。 - QUIC监听器仅支持RSA证书。 - 非HTTPS协议的监听器传入sni_container_refs字段将被忽略并返回[]。
        ...
        :param security_policy_id: 自定义安全策略的ID。 [仅对HTTPS协议类型的监听器且关联LB为独享型时有效。](tag:hws,hws_hk,ocb,tlf,ctc,hcso,sbc,g42,tm,cmcc,hk-g42,dt,dt_test) [仅对HTTPS协议类型的监听器有效](tag:hcso_dt) 使用说明： - 若同时设置了security_policy_id和tls_ciphers_policy，则仅security_policy_id生效。
        :type security_policy_id: str
        :param enable_member_retry: 是否开启后端服务器的重试。取值：true 开启重试，false 不开启重试。  [使用说明： - 若关联是共享型LB，仅在protocol为HTTP、TERMINATED_HTTPS时才能传入该字段。 - 若关联是独享型LB，仅在protocol为HTTP、HTTPS、QUIC时才能传入该字段。](tag:hws,hws_hk,ocb,tlf,ctc,hcso,sbc,g42,tm,cmcc,hk-g42,dt,dt_test) [使用说明： - 仅在protocol为HTTP、HTTPS、QUIC时才能传入该字段。](tag:hcso_dt)
        :type enable_member_retry: bool
        :param member_timeout: 等待后端服务器响应超时时间。请求转发后端服务器后，在等待超时member_timeout时长没有响应，负载均衡将终止等待，并返回 HTTP504错误码。  取值：1-300s。  使用说明： - 仅支持协议为HTTP/HTTPS/QUIC的监听器。
        :type member_timeout: int
        :param client_timeout: 等待客户端请求超时时间，仅限协议为HTTP/HTTPS/QUIC的监听器配置。取值范围为1-300s, 默认值为60s TCP，UDP协议的监听器不支持此字段
        :type client_timeout: int
        :param keepalive_timeout: 客户端连接空闲超时时间。在超过keepalive_timeout时长一直没有请求，负载均衡会暂时中断当前连接，直到一下次请求时重新建立新的连接。取值：  - TCP/UDP监听器：10-4000s。  - HTTP/HTTPS/TERMINATED_HTTPS/QUIC监听器：0-4000s。  - UDP监听器不支持此字段。
