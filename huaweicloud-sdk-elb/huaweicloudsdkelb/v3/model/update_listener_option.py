...
class UpdateListenerOption:
    ...
    def __init__(self, admin_state_up=None, client_ca_tls_container_ref=None, default_pool_id=None, default_tls_container_ref=None, description=None, http2_enable=None, insert_headers=None, name=None, sni_container_refs=None, sni_match_algo=None, tls_ciphers_policy=None, security_policy_id=None, enable_member_retry=None, member_timeout=None, client_timeout=None, keepalive_timeout=None, ipgroup=None, transparent_client_ip_enable=None, proxy_protocol_enable=None, enhance_l7policy_enable=None, quic_config=None, protection_status=None, protection_reason=None, gzip_enable=None, ssl_early_data_enable=None, cps=None, connection=None, nat64_enable=None):
        ...
        :param admin_state_up: **参数解释**：监听器的管理状态。  **约束限制**：只支持更新为true。  **取值范围**：不涉及  **默认取值**：不涉及 [不支持该字段，请勿使用。](tag:dt,hcso_dt)
        :type admin_state_up: bool
        :param client_ca_tls_container_ref: **参数解释**：监听器使用的CA证书ID。  **约束限制**： - 当且仅当type&#x3D;client时，才会使用该字段对应的证书。 - 监听器协议为QUIC时不支持该字段。  **取值范围**：不涉及  **默认取值**：不涉及  [不支持QUIC。](tag:tm,hws_eu,g42,hk_g42,hcso_dt,dt)
        :type client_ca_tls_container_ref: str
        :param default_pool_id: **参数解释**：监听器的默认后端服务器组ID。当请求没有匹配的转发策略时，转发到默认后端服务器组上处理。  **约束限制**：不涉及  **取值范围**：不涉及  **默认取值**：不涉及
        :type default_pool_id: str
        :param default_tls_container_ref: **参数解释**：监听器使用的服务器证书ID。  **约束限制**：当监听器协议为HTTPS时，该字段必传，且对应的证书的type必须是server类型。  **取值范围**：不涉及  **默认取值**：不涉及
        :type default_tls_container_ref: str
        :param description: **参数解释**：监听器的描述信息。  **约束限制**：不涉及  **取值范围**：不涉及  **默认取值**：不涉及
        :type description: str
        :param http2_enable: **参数解释**：客户端与LB之间的HTTPS请求的HTTP2功能的开启状态。 开启后，可提升客户端与LB间的访问性能，但LB与后端服务器间仍采用HTTP1.X协议。  **约束限制**： - 仅HTTPS协议监听器有效。 - QUIC监听器不能设置该字段，固定返回为true。 - 其他协议的监听器可设置该字段但无效，无论取值如何都不影响监听器正常运行。  **取值范围**：不涉及  **默认取值**：不涉及  [不支持QUIC。](tag:tm,hws_eu,g42,hk_g42,hcso_dt,dt)
        :type http2_enable: bool
        :param insert_headers: 
        :type insert_headers: :class:`huaweicloudsdkelb.v3.ListenerInsertHeaders`
        :param name: **参数解释**：监听器的名称。  **约束限制**：若名称为空，则在控制台的监听器列表无法选择并查看监听器详情。  **取值范围**：不涉及  **默认取值**：不涉及
        :type name: str
        :param sni_container_refs: **参数解释**：监听器使用的SNI证书（带域名的服务器证书）ID列表。  **约束限制**： - QUIC监听器仅支持RSA证书。 - 列表对应的所有SNI证书的域名不允许存在重复。 - 列表对应的所有SNI证书的域名总数不超过200。  **取值范围**：不涉及  **默认取值**：不涉及  [不支持QUIC。](tag:tm,hws_eu,g42,hk_g42,hcso_dt,dt)
        :type sni_container_refs: list[str]
        :param sni_match_algo: **参数解释**：监听器使用的SNI证书泛域名匹配方式。  **约束限制**：不涉及  **取值范围**： - longest_suffix表示最长尾缀匹配。 - wildcard表示标准域名分级匹配。  **默认取值**：wildcard。
        ...
        :param gzip_enable: **参数解释**：ELB是否开启gzip压缩。  [**约束限制**：仅HTTP/HTTPS类型监听器支持配置。](tag:tm,hws_eu,g42,hk_g42,hcso_dt,dt) [**约束限制**：仅HTTP/HTTPS/QUIC类型监听器支持配置。](tag:hws,hws_hk,hws_test,hcs,hcs_sm,hcso,hk_vdf,srg,fcs,fcs_vm,mix,ocb,ctc,cmcc,ct,sbc,hws_ocb,hk_sbc)  **取值范围**：不涉及  **默认取值**：false
        :type gzip_enable: bool
        :param ssl_early_data_enable: **参数解释**：监听器0-RTT能力开关。  **约束限制**：仅HTTPS类型监听器支持配置，需要依赖TLSv1.3安全策略协议。开启 ssl_early_data 后，有重放攻击的风险，请谨慎开启。  **取值范围**：不涉及  **默认取值**：false
