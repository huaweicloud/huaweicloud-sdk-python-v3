...
class UpdateRedirectUrlConfig:
    ...
    def __init__(self, protocol=None, host=None, port=None, path=None, query=None, status_code=None, insert_headers_config=None, remove_headers_config=None):
        ...
        :param protocol: **参数解释**：重定向的协议。  **约束限制**：不涉及  **取值范围**： - HTTP - HTTPS - ${protocol}，表示继承原值（即与被转发请求保持一致）。  **默认取值**：不涉及
        :type protocol: str
        ...
        :type port: str
        :param path: **参数解释**：重定向的路径。  **约束限制**：不涉及  **取值范围**：只能包含英文字母、数字、_~&#39;;@^-%#&amp;$.\\*+?,&#x3D;!:|\\/()\\[\\]{}，且必须以\&quot;/\&quot;开头。  **默认取值**：${path}，表示继承原值（即与被转发请求保持一致）。
        :type path: str
