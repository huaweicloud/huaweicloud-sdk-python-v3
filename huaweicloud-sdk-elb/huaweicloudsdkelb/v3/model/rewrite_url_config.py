...
class RewriteUrlConfig:
    ...
    def __init__(self, host=None, path=None, query=None):
        ...
        :param host: **参数解释**：重定向的域名。  **取值范围**：英文字母、数字、“-”、“.”，必须以字母、数字开头。
        :type host: str
        :param path: **参数解释**：重定向的请求路径。其中$1-$9会匹配请求url通配符星号(*)，当正则匹配分组小于指定数字，则$指定数字结果为空。$后面跟字母，匹配结果均为空，直到下一个特殊字符出现，例如$abc#123，则匹配结果为#123；$后面跟特殊字符则直接输出特殊字符，例如$#匹配结果为$#。  **取值范围**：英文字母、数字、_~&#39;;@^-%#&amp;$.+?,&#x3D;!:|/()，且必须以\&quot;/\&quot;开头。
        :type path: str
        :param query: **参数解释**：重定向的查询字符串。其中$1-$9会匹配请求url通配符星号（*），当正则匹配分组小于指定数字，则$指定数字结果为空。$后面跟字母，匹配结果均为空，直到下一个特殊字符出现，例如$abc#123，则匹配结果为#123；$后面跟特殊字符则直接输出特殊字符，例如$#匹配结果为$#。  **取值范围**：英文字母、数字和特殊字符：!$&amp;&#39;()+,-./:;&#x3D;?@^_&#x60;。字母区分大小写。
        :type query: str
        """
    ...
    @property
    def host(self):
        r"""Gets the host of this RewriteUrlConfig.
    ...
    @host.setter
    def host(self, host):
        r"""Sets the host of this RewriteUrlConfig.
    ...
    @property
    def path(self):
        r"""Gets the path of this RewriteUrlConfig.
    ...
    @path.setter
    def path(self, path):
        r"""Sets the path of this RewriteUrlConfig.
    ...
    @property
    def query(self):
        r"""Gets the query of this RewriteUrlConfig.
    ...
    def query(self, query):
        r"""Sets the query of this RewriteUrlConfig.
