...
class UpdateLoadbalancerAutoscalingOption:
    ...
    def __init__(self, enable=None, min_l7_flavor_id=None):
        ...
        :param enable: **参数解释**：当前负载均衡器是否开启弹性扩缩容。  **约束限制**：不涉及  **取值范围**： - true：开启。 - false：不开启。  **默认取值**：不涉及
        :type enable: bool
        :param min_l7_flavor_id: **参数解释**：弹性扩缩容的最小七层规格ID（规格类型L7_elastic）。  **约束限制**：有七层监听器时，该字段不能为空。  **取值范围**：不涉及  **默认取值**：不涉及
    ...
    def enable(self):
        ...
        **参数解释**：当前负载均衡器是否开启弹性扩缩容。  **约束限制**：不涉及  **取值范围**： - true：开启。 - false：不开启。  **默认取值**：不涉及
    ...
    def min_l7_flavor_id(self):
        ...
        **参数解释**：弹性扩缩容的最小七层规格ID（规格类型L7_elastic）。  **约束限制**：有七层监听器时，该字段不能为空。  **取值范围**：不涉及  **默认取值**：不涉及
