...
class CreateMasterSlaveMemberOption:
    ...
    def __init__(self, address=None, admin_state_up=None, name=None, protocol_port=None, subnet_cidr_id=None, role=None):
        ...
        :param subnet_cidr_id: 后端云服务器所在的子网ID，可以是子网的IPv4子网ID或IPv6子网ID。  使用说明： 1. 该子网和关联的负载均衡器的子网必须在同一VPC下。 2. 若所属LB的跨VPC后端转发特性已开启，则该字段可以不传，表示添加跨VPC的后端服务器。此时address必须为IPv4地址，所在的pool的协议必须为TCP/HTTP/HTTPS。 [不支持IPv6，请勿设置为IPv6子网ID。](tag:dt,dt_test)
