...
class CreateLoadBalancerOption:
    ...
    def __init__(self, project_id=None, name=None, description=None, vip_address=None, vip_subnet_cidr_id=None, ipv6_vip_virsubnet_id=None, provider=None, l4_flavor_id=None, l7_flavor_id=None, guaranteed=None, vpc_id=None, availability_zone_list=None, enterprise_project_id=None, tags=None, admin_state_up=None, billing_info=None, ipv6_bandwidth=None, publicip_ids=None, publicip=None, elb_virsubnet_ids=None, ip_target_enable=None, deletion_protection_enable=None, prepaid_options=None, autoscaling=None, waf_failure_action=None, protection_status=None, protection_reason=None, charge_mode=None, ipv6_vip_address=None):
        ...
        :param project_id: 参数解释：负载均衡器实例所在的项目ID。
        :type project_id: str
        :param name: 参数解释：负载均衡器的名称。  约束限制： 可以为空, 最大长度为255个字符。  取值范围：支持中文字符、英文字符等unicode字符，且长度为[0-255]个字符。
        :type name: str
        :param description: 参数解释：负载均衡器的描述。  约束限制： 可以为空, 最大长度为255个字符。  取值范围：支持中文字符、英文字符等unicode字符，且长度为[0-255]个字符。
        :type description: str
        :param vip_address: 参数解释：负载均衡器的IPv4私网IP。该地址必须包含在所在子网的IPv4网段内，且未被占用。  约束限制： - 传入vip_address时，必须传入vip_subnet_cidr_id。 - 不传入vip_address，但传入vip_subnet_cidr_id，则自动分配IPv4私网IP。 - 不传入vip_address，且不传vip_subnet_cidr_id，则不分配IPv4私网IP，vip_address&#x3D;null。  取值范围：满足IPv4的地址格式，[0-255].[0-255].[0-255].[0-255]. 如192.168.1.1。
        ...
        :type vpc_id: str
        :param availability_zone_list: 参数解释：负载均衡器实例所在的可用区列表。  约束限制： - 参数获取，可通过GET https://{ELB_Endponit}/v3/{project_id}/elb/availability-zones 接口来查询可用区集合列表。创建负载均衡器时，从查询结果选择某一个可用区集合，并从中选择一个或多个可用区。 - 为了支持可用区容灾，建议选取不少于2个可用区。
        ...
        :param admin_state_up: 参数解释：负载均衡器的启用状态。  取值范围：布尔类型。 - true ：启用。 - false：停用。  默认取值：true。  [不支持该字段，请勿使用。](tag:dt,dt_test)
        :type admin_state_up: bool
        :param billing_info: 参数解释: 预付费实例的订单信息。  约束限制： - 空：按需计费。 [- 非空：包周期计费。 格式为： order_id:product_id:region_id:project_id，如： CS2107161019CDJZZ:OFFI569702121789763584:region-xxx:057ef081eb00d2732fd1c01a9be75e6f   不支持该字段，请勿使用。](tag:hws_eu,g42,hk_g42,dt,dt_test,hcso_dt,hcso,hk_vdf,fcs,fcs_vm,mix,hcso_g42,hcso_g42_b)
        :type billing_info: str
        :param ipv6_bandwidth: 
        :type ipv6_bandwidth: :class:`huaweicloudsdkelb.v3.BandwidthRef`
        :param publicip_ids: 参数解释：负载均衡器绑定的公网IP的ID的数组。  约束限制：只支持绑定数组中的第一个EIP，其他将被忽略。
        :type publicip_ids: list[str]
        :param publicip: 
        :type publicip: :class:`huaweicloudsdkelb.v3.CreateLoadBalancerPublicIpOption`
        :param elb_virsubnet_ids: 参数解释：负载均衡器实例所在VPC的子网的网络ID列表。 负载均衡器实例，会预占该子网中的部分IP， 用于负载均衡器网关与该实例后端服务器通信的源地址（典型场景，健康检查探测的源地址，FULLNAT场景的源地址等）。 因此，该子网，也称为负载均衡器的后端子网。通常需要用户指定一个特殊的子网，方便用户在后端服务器关联的安全组中，放通该子网的地址段。  约束限制： 后端子网必须属于该负载均衡器实例所在的VPC。 参数的获取，使用负载均衡器所在vpc的ID查询可用子网 可以通过GET https://{VPC_Endpoint}/v1/{project_id}/subnets?vpc_id&#x3D;xxxx 响应参数中的neutron_network_id得到。  若不指定该字段，则按如下规则选择下联面网络： 1. 如果ELB实例开启ipv6，则选择ipv6_vip_virsubnet_id子网对应的网络ID。 2. 如果ELB实例没有开启ipv6，开启ipv4，则选择vip_subnet_cidr_id子网对应的网络ID。 3. 如果ELB实例没有选择私网，只开启公网，则会在当前负载均衡器所在的VPC中任意选一个子网，优选可用IP多的网络。  若指定多个下联面子网，则按顺序优先使用第一个子网来为负载均衡器下联面端口分配ip地址。  建议后端子网，使用一个独占的地址充足的子网，方便运维管理。 - 后端服务器安全组放通：由于负载均衡器网关会使用该子网中的预占的地址，作为源IP与后端服务器通信（健康检查探测，FULLNAT通信），为避免后端服务器关联的安全组拦截，建议将对应的子网地址段进行安全组放通。 - 预占地址变化：负载均衡实例，弹性扩缩场景，可能涉及到预占地址的变化，建议安全组对子网段放通，而不是具体预占地址的放通。
        :type elb_virsubnet_ids: list[str]
        :param ip_target_enable: 参数解释：是否启用跨VPC后端转发。  约束限制： 开启跨VPC后端转发后，后端服务器组不仅支持添加云上VPC内的服务器，还支持添加其他VPC、其他公有云、云下数据中心的服务器。 使用共享VPC的实例使用此特性时，需确保共享资源所有者已开通VPC对等连接，否则通信异常。 [仅独享型负载均衡器支持该特性。 ](tag:hws,hws_hk,ocb,ctc,hcs,g42,tm,cmcc,hk_g42,hws_ocb,hk_vdf,fcs,dt) - 开启不能关闭。 [荷兰region不支持该字段，请勿使用。](tag:dt)  取值范围： - true：开启。 - false：不开启。
        ...
        :param waf_failure_action: 参数解释：WAF故障时的流量处理策略。  约束限制：只有绑定了waf的LB实例，该字段才会生效。 [不支持该字段，请勿使用。](tag:hws_hk,hws_eu,hws_test,hcs,hcs_sm,hcso,hk_vdf,fcs,fcs_vm,mix,hcso_g42,hcso_g42_b,hcso_dt,dt,dt_test,ocb,ctc,cmcc,tm,sbc,g42,hws_ocb,hk_sbc,hk_tm,hk_g42)  取值范围： - discard:丢弃。 - forward: 转发到后端。  默认取值：forward。
        ...
        :param protection_reason: 参数解释：设置保护的原因。作为protection_status的转态设置的原因。  约束限制：仅当protection_status为consoleProtection时有效。  取值范围：通用Unicode字符集字符，最大255个字符。
        :type protection_reason: str
        :param charge_mode: 参数解释：负载均衡器实例的计费模式。flavor: 固定规格计费，默认值。lcu: 弹性规格计费（按用户真实使用的lcu个数计费）。  约束限制：不建议用户传该字段。系统会基于用户传入的l4_flavor_id/l7_flavor_id的规格类型，自动识别计费模式。  取值范围：    - flavor: 固定规格计费。    - lcu: 弹性规格计费（按用户真实使用的lcu个数计费）。  默认取值：flavor。
        :type charge_mode: str
        :param ipv6_vip_address: 参数解释：负载均衡器实例的IPv6地址。  约束限制：  - 必须属于ipv6_vip_virsubnet_id子网中地址。  - elb_virsubnet_ids中的后端子网必须支持双栈。  [不支持IPv6，请勿使用。](tag:dt,dt_test)
    ...
    def protection_status(self):
        """Gets the protection_status of this CreateLoadBalancerOption.
    ...
    def protection_status(self, protection_status):
        """Sets the protection_status of this CreateLoadBalancerOption.
    ...
    def protection_reason(self):
        """Gets the protection_reason of this CreateLoadBalancerOption.
    ...
    def protection_reason(self, protection_reason):
        """Sets the protection_reason of this CreateLoadBalancerOption.
    ...
    def charge_mode(self):
        """Gets the charge_mode of this CreateLoadBalancerOption.
    ...
    def charge_mode(self, charge_mode):
        """Sets the charge_mode of this CreateLoadBalancerOption.
